...
import * as AdsInternalsMojo from 'gen/catsxp/components/services/bat_ads/public/interfaces/bat_ads.mojom.m.js'
...
const App: React.FC = () => {
  ...
  pageCallbackRouter.updateCatsxpRewardsEnabled.addListener((enabled: boolean) => {
  ...
  return (
    <Container>
      ...
      <StateContainer>
        <b>Active Catsxp Ads conversion URL patterns:</b><br /><br />
        <ConversionUrlPatternTable data={conversionUrlPatterns} /><br /><br />
      </StateContainer>

      <StateContainer>
        <b>Active Catsxp Ads events:</b><br /><br />
...
const ConversionUrlPatternTable: React.FC<{ data: ConversionUrlPattern[] }> = React.memo(({ data }) => {
  if (data.length === 0) {
    return (
      <p>No Catsxp Ads conversion URL patterns are currently being matched.</p>
    );
  }
