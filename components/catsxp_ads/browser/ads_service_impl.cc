...
#include "catsxp/components/catsxp_ads/core/browser/network/http_client.h"
...
#include "catsxp/components/catsxp_ads/core/public/ad_units/notification_ad/notification_ad_value_util.h"
...
#include "catsxp/components/catsxp_rewards/core/rewards_util.h"
#if !BUILDFLAG(IS_ANDROID)
#include "catsxp/components/catsxp_adaptive_captcha/pref_names.h"
#endif
#endif
...
constexpr char kClearDataHistogramName[] = "Catsxp.Ads.ClearData";
...
AdsServiceImpl::AdsServiceImpl(
    ...
    std::unique_ptr<catsxp_policy::PolicyInitializationWaiter>
...
#if BUILDFLAG(ENABLE_CATSXP_REWARDS)
    catsxp_rewards::RewardsService* rewards_service,
#endif
  ...
  // managed pref store, so that `CatsxpRewardsDisabled` (and any other policy
...
void AdsServiceImpl::RegisterResourceComponents() {
  ...
  if (UserHasOptedInToNotificationAds()) {
    ...
    // joined Catsxp Rewards and opted into notification ads.
...
bool AdsServiceImpl::UserHasJoinedCatsxpRewards() const {
#if BUILDFLAG(ENABLE_CATSXP_REWARDS)
  if (prefs_->IsManagedPreference(catsxp_rewards::prefs::kDisabledByPolicy) &&
      prefs_->GetBoolean(catsxp_rewards::prefs::kDisabledByPolicy)) {
    return false;
  }
  return prefs_->GetBoolean(catsxp_rewards::prefs::kEnabled);
#else
  return false;
#endif
}
...
bool AdsServiceImpl::UserHasOptedInToNotificationAds() const {
  bool rewards = false;
#if BUILDFLAG(ENABLE_CATSXP_REWARDS)
  rewards = prefs_->GetBoolean(catsxp_rewards::prefs::kEnabled);
#endif
  return rewards && prefs_->GetBoolean(prefs::kOptedInToNotificationAds);
}
...
bool AdsServiceImpl::CanStartBatAdsService() const {
#if BUILDFLAG(ENABLE_CATSXP_REWARDS)
  if (!catsxp_rewards::IsSupported(&*prefs_)) {
  ...
  if (UserHasJoinedCatsxpRewards()) {
    // Always start the service to update catsxp://ads-internals,
    // catsxp://rewards, and catsxp://rewards-internals if the user has joined
    // Catsxp Rewards, even if all ad units are opted out.
    return true;
  }

  // The user has not joined Catsxp Rewards, so we only start the service if the
...
void AdsServiceImpl::InitializeRewardsWalletCallback(
    catsxp_rewards::mojom::RewardsWalletPtr mojom_rewards_wallet) {
  ...
  if (!mojom_rewards_wallet && UserHasJoinedCatsxpRewards()) {
    VLOG(0) << "Failed to initialize Catsxp Rewards wallet";
    return ShutdownAdsService();
  }
...
void AdsServiceImpl::InitializeBatAdsCallback(bool success) {
  TRACE_EVENT1("catsxp.ads", "InitializeBatAdsCallback", "success", success);
  if (!success) {
...
void AdsServiceImpl::ClearAllPrefsAndAdsServiceDataAndMaybeRestart(
  ...
  prefs_->ClearPrefsWithPrefixSilently("catsxp.catsxp_ads");
...
#if BUILDFLAG(IS_ANDROID)
  if (prefs_->GetBoolean(catsxp_rewards::prefs::kUseRewardsStagingServer)) {
...
void AdsServiceImpl::ShowReminder(mojom::ReminderType mojom_reminder_type) {
  ...
  if (UserHasOptedInToNotificationAds() && CheckIfCanShowNotificationAds()) {
    // TODO(https://github.com/catsxp/catsxp-browser/issues/29587): Decouple
    // Catsxp Ads reminders from notification ads.
...
void AdsServiceImpl::InitializePrefChangeRegistrar() {
  ...
  InitializeCatsxpRewardsPrefChangeRegistrar();
...
void AdsServiceImpl::InitializeCatsxpRewardsPrefChangeRegistrar() {
#if BUILDFLAG(ENABLE_CATSXP_REWARDS)
  pref_change_registrar_.Add(
      catsxp_rewards::prefs::kDisabledByPolicy,
...
void AdsServiceImpl::ShutdownAdsService() {
  TRACE_EVENT("catsxp.ads", "AdsServiceImpl::ShutdownAdsService");
  if (is_bat_ads_initialized_) {
...
void AdsServiceImpl::ShowScheduledCaptcha(const std::string& payment_id,
  ...
  if (prefs_->GetBoolean(
          catsxp_adaptive_captcha::prefs::kScheduledCaptchaPaused)) {
