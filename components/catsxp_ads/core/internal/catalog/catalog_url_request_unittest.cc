...
#include "catsxp/components/catsxp_ads/core/internal/catalog/catalog_url_request.h"
...
#include "catsxp/components/catsxp_ads/core/internal/catalog/test/catalog_url_request_delegate_mock.h"
...
class CatsxpAdsCatalogUrlRequestTest : public test::TestBase {
 protected:
  void SetUp() override {
    test::TestBase::SetUp();
...
TEST_F(CatsxpAdsCatalogUrlRequestTest,
...
TEST_F(
    CatsxpAdsCatalogUrlRequestTest,
...
TEST_F(CatsxpAdsCatalogUrlRequestTest, DoNotFetchIfAlreadyPeriodicallyFetching) {
  // Arrange
  const std::optional<std::string> response_body =
      test::MaybeReadFileToStringAndReplaceTags(
          test::kCatalogWithSingleCampaignJsonFilename);
