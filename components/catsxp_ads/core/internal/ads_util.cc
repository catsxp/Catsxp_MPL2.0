...
#include "catsxp/components/l10n/common/ofac_sanction_util.h"
...
#if BUILDFLAG(IS_ANDROID)
  // TODO(https://github.com/catsxp/catsxp-browser/issues/52800): Remove
  // `kUseRewardsStagingServer` pref and use command-line flag on Android
  // instead.
  if (prefs.GetBoolean(catsxp_rewards::prefs::kUseRewardsStagingServer)) {
...
bool IsSupportedRegion() {
  return !catsxp_l10n::IsISOCountryCodeOFACSanctioned(CurrentCountryCode());
}
