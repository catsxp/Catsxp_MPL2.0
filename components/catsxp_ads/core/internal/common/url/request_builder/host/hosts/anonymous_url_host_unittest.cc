...
class CatsxpAdsAnonymousUrlHostTest : public test::TestBase {};

TEST_F(CatsxpAdsAnonymousUrlHostTest, GetProductionUrlHost) {
  // Arrange
...
TEST_F(CatsxpAdsAnonymousUrlHostTest, GetStagingUrlHost) {
  // Arrange
  ASSERT_TRUE(GlobalState::HasInstance());
  GlobalState::GetInstance()->CommandLineSwitches().environment_type =
  ...
  // Act & Assert
  EXPECT_EQ("https://anonymous.ads.catsxpsoftware.com",
            GetUrlHost(UrlHostType::kAnonymous));
}
