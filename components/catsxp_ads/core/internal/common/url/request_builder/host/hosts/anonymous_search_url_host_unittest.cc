...
class CatsxpAdsAnonymousSearchUrlHostTest : public test::TestBase {};

TEST_F(CatsxpAdsAnonymousSearchUrlHostTest, GetProductionUrlHost) {
  // Arrange
...
TEST_F(CatsxpAdsAnonymousSearchUrlHostTest, GetStagingUrlHost) {
  // Arrange
  ASSERT_TRUE(GlobalState::HasInstance());
  GlobalState::GetInstance()->CommandLineSwitches().environment_type =
  ...
  // Act & Assert
  EXPECT_EQ("https://search.anonymous.ads.catsxpsoftware.com",
            GetUrlHost(UrlHostType::kAnonymousSearch));
}
