...
TEST(CatsxpAdsSearchEngineResultsPageUrlPatternConstantsTest,
...
TEST(CatsxpAdsSearchEngineResultsPageUrlPatternConstantsTest,
  ...
  EXPECT_EQ(
      ...
      "sh|si|sk|sm|sn|so|sr|st|td|tg|tk|tl|tm|tn|to|tt|vg|vu|ws)/search",
      GetGoogleResultsPageUrlPattern());
}
...
TEST(CatsxpAdsSearchEngineResultsPageUrlPatternConstantsTest,
  ...
  // Act & Assert
  EXPECT_EQ("https://www.mojeek.(co.uk|com)/search",
            GetMojeekResultsPageUrlPattern());
}
