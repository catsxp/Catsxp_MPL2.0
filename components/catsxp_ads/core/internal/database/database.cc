...
void Database::ErrorCallback(int extended_error,
  ...
  if (!sql::Database::IsExpectedSqliteError(extended_error)) {
    ...
    if (!IsTransientSqliteError(result_code)) {
      SCOPED_CRASH_KEY_NUMBER("CatsxpAds", "sqlite_schema_version",
                              database::kVersionNumber);
      SCOPED_CRASH_KEY_STRING1024(
          "CatsxpAds", "sqlite_diagnostic_info",
          db_.GetDiagnosticInfo(extended_error, statement));
      SCOPED_CRASH_KEY_STRING1024("CatsxpAds", "sqlite_error_message",
                                  db_.GetErrorMessage());
      SCOPED_CRASH_KEY_NUMBER("CatsxpAds", "sqlite_result_code",
