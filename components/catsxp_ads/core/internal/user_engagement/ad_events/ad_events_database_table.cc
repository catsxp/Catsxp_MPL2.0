...
void MigrateToV50(const mojom::DBTransactionInfoPtr& mojom_db_transaction) {
  ...
  Execute(mojom_db_transaction, R"(
      CREATE TABLE ad_events_temp (
        ...
        advertiser_id TEXT,
        segment TEXT,
        target_url TEXT NOT NULL DEFAULT 'https://catsxp.com/catsxp-ads/',
...
void AdEvents::PurgeExpired(ResultCallback callback) const {
  ...
  const size_t non_new_tab_page_ad_days = UserHasJoinedCatsxpRewards() ? 90 : 30;
  ...
  const size_t new_tab_page_ad_days =
      UserHasJoinedCatsxpRewards() || UserHasOptedInToSurveyPanelist() ? 90 : 2;
