...
class CatsxpAdsLinearTest : public test::TestBase {
 public:
...
TEST_F(CatsxpAdsLinearTest, ThreeClassesPredictionTest) {
  // Arrange
...
TEST_F(CatsxpAdsLinearTest, BiasesPredictionTest) {
  // Arrange
  const std::map<std::string, VectorData> weights = {
      {"class_1", VectorData({1.0, 0.0, 0.0})},
      {"class_2", VectorData({0.0, 1.0, 0.0})},
...
TEST_F(CatsxpAdsLinearTest, BinaryClassifierPredictionTest) {
  // Arrange
...
TEST_F(CatsxpAdsLinearTest, TopPredictionsTest) {
  // Arrange
