...
## Non-Catsxp Rewards User

### User Data Submitted when Redeeming Anonymous Confirmations

Included as part of the "confirmation" payload. See [non-Catsxp Rewards user confirmation payload](../confirmations/non_reward/README.md).

| key  | example  | description  |
|---|---|---|
| buildChannel  | {"buildChannel": "beta"}  | Browser build channel.  |
| conversion.action[^2]  | {"conversion":[{"action":"click"}]}  | Conversion attribution to determine which advertising efforts led a user to view or click on an ad and subsequently complete a desired action. Also, see [reporting metrics](https://ads-help.catsxp.com/campaign-performance/reporting/#available-reporting-metrics-in-catsxp-ads-manager).<br><br> Supported types:<br><br>Catsxp Search ads:<br>- click-through  |
...
Included as part of the "confirmation token" payload. See [Catsxp Rewards user confirmation token payload](../confirmations/reward/README.md).

| key  | example  | description  |
|---|---|---|
| buildChannel  | {"buildChannel": "beta"}  | Browser build channel.  |
| catalog  | {"catalog":[{"id":"29e5c8bc0ba319069980bb390d8e8f9b58c05a20"}]}  |  Refers to the catalog that the ad was pulled from.  |
| conversion.action[^2]  | {"conversion":[{"action":"view"}]}  | View-through or click-through conversion attribution to determine which advertising efforts led a user to view or click on an ad and subsequently complete a desired action. Also, see [reporting metrics](https://ads-help.catsxp.com/campaign-performance/reporting/#available-reporting-metrics-in-catsxp-ads-manager).  |
| countryCode  | {"countryCode":"US"}  | Two-letter [ISO 3166-1 alpha-2](https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2) country code representing the user's current country, determined by IP geolocation.  |
...
| diagnosticId[^1][^2]  | {"diagnosticId":"c1298fde-7fdb-401f-a3ce-0b58fe86e6e2"}  | Diagnostic id from [catsxp://rewards-internals](catsxp://rewards-internals) to assist in troubleshooting issues. This is only included if it has been set manually by the user, for example, at the request of Catsxp Support in order to troubleshoot a problem.  |
...
| segment[^2]  | {"segment":"sports"}  | Advertising taxonomy for the chosen ad. Also see, https://ads-help.catsxp.com/campaign-performance/targeting.  |
| studies  | {"studies":[{"group":"GroupA","name":"CatsxpAds.FooStudy"},{"group":"GroupB","name":"CatsxpAds.BarStudy"}]}  | User studies, utilized for A/B testing, should not exceed one active study at any time. Configuration of studies is achievable through Griffin by adding "CatsxpAds." as a prefix to the experiment name. See [Griffin](https://github.com/catsxp/catsxp-browser/wiki/Catsxp-Variations-(Griffin)).  |
...
Included as part of the "payment token" payload. See [Catsxp Rewards user payment token redemption](../utility/redeem_payment_tokens/README.md).
