...
class CatsxpAdsIssuersUrlRequestJsonReaderUtilTest : public test::TestBase {};

TEST_F(CatsxpAdsIssuersUrlRequestJsonReaderUtilTest, ParsePing) {
  // Arrange
...
TEST_F(CatsxpAdsIssuersUrlRequestJsonReaderUtilTest, DoNotParseMissingPing) {
  // Act & Assert
  EXPECT_FALSE(json::reader::ParsePing({}));
}

TEST_F(CatsxpAdsIssuersUrlRequestJsonReaderUtilTest, ParseTokenIssuers) {
  // Arrange
  const base::DictValue dict = base::test::ParseJsonDict(R"JSON(
    {
      "issuers": [
...
TEST_F(CatsxpAdsIssuersUrlRequestJsonReaderUtilTest,
