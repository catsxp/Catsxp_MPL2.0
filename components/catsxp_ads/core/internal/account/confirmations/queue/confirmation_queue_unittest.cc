...
#include "catsxp/components/catsxp_ads/core/internal/account/confirmations/queue/queue_item/confirmation_queue_item_util.h"
...
#include "catsxp/components/catsxp_ads/core/internal/account/confirmations/queue/test/confirmation_queue_delegate_mock.h"
...
class CatsxpAdsConfirmationQueueTest : public test::TestBase {
 protected:
  void SetUp() override {
    test::TestBase::SetUp();
...
TEST_F(CatsxpAdsConfirmationQueueTest, AddNonRewardConfirmation) {
  // Arrange
...
// schedules it for delayed processing, and leaves a pending task to process the
// queued confirmation later.
TEST_F(CatsxpAdsConfirmationQueueTest, AddRewardConfirmation) {
  // Arrange
...
// processed/exhausted. Catsxp Rewards must be disabled for this test because
...
TEST_F(CatsxpAdsConfirmationQueueTest, ProcessNonRewardConfirmation) {
  // Arrange
  test::DisableCatsxpRewards();
...
// confirmations have been processed/exhausted. Catsxp Rewards must be disabled
...
TEST_F(CatsxpAdsConfirmationQueueTest, ProcessMultipleNonRewardConfirmations) {
  // Arrange
  test::DisableCatsxpRewards();
