...
|---|---|---|---|
| new tab page ad  | yes¹ ²  | Always, but the frequency is capped when the user has joined Catsxp Rewards.  | Displayed on new tab pages  |
| notification ad  | yes²  | Only when opted into notification ads  | Displayed as system or custom push notifications  |
| search result ad  | no² ³  | Always, but the frequency is capped when the user has joined Catsxp Rewards.  | Displayed on [search.catsxp.com](search.catsxp.com).  |

Users are rewarded for viewed ad impressions if they join Catsxp Rewards.
