/* Copyright (c) 2020 The Catsxp Authors. All rights reserved. */

#include "catsxp/components/catsxp_ads/core/public/ad_units/notification_ad/notification_ad_value_util.h"

#include <algorithm>
#include <string_view>

#include "base/containers/circular_deque.h"
#include "base/test/values_test_util.h"
#include "catsxp/components/catsxp_ads/core/internal/ad_units/test/ad_test_constants.h"
#include "catsxp/components/catsxp_ads/core/internal/common/test/test_base.h"
#include "catsxp/components/catsxp_ads/core/internal/creatives/notification_ads/creative_notification_ad_info.h"
#include "catsxp/components/catsxp_ads/core/internal/creatives/notification_ads/notification_ad_builder.h"
#include "catsxp/components/catsxp_ads/core/internal/creatives/notification_ads/test/creative_notification_ad_test_util.h"
#include "catsxp/components/catsxp_ads/core/public/ad_units/notification_ad/notification_ad_info.h"

// npm run test -- catsxp_unit_tests --filter=CatsxpAds*

namespace catsxp_ads {

namespace {

constexpr std::string_view kNotificationAdAsJson =
    R"JSON(
        {
          "advertiser_id": "5484a63f-eb99-4ba5-a3b0-8c25d3c0e4b2",
          "body": "Test Ad Body",
          "campaign_id": "84197fc8-830a-4a8e-8339-7a70c2bfa104",
          "creative_instance_id": "546fe7b0-5047-4f28-a11c-81f14edcf0f6",
          "creative_set_id": "c2ba3e7d-f688-4bc4-a053-cbe7ac1e6123",
          "segment": "untargeted",
          "target_url": "https://catsxp.com/",
          "title": "Test Ad Title",
          "type": "ad_notification",
          "uuid": "9bac9ae4-693c-4569-9b3e-300e357780cf"
        })JSON";

constexpr std::string_view kNotificationAdsAsJson =
    R"JSON(
        [
          {
            "advertiser_id": "5484a63f-eb99-4ba5-a3b0-8c25d3c0e4b2",
            "body": "Test Ad Body",
            "campaign_id": "84197fc8-830a-4a8e-8339-7a70c2bfa104",
            "creative_instance_id": "546fe7b0-5047-4f28-a11c-81f14edcf0f6",
            "creative_set_id": "c2ba3e7d-f688-4bc4-a053-cbe7ac1e6123",
            "segment": "untargeted",
            "target_url": "https://catsxp.com/",
            "title": "Test Ad Title",
            "type": "ad_notification",
            "uuid": "9bac9ae4-693c-4569-9b3e-300e357780cf"
          },
          {
            "advertiser_id": "5484a63f-eb99-4ba5-a3b0-8c25d3c0e4b2",
            "body": "Test Ad Body",
            "campaign_id": "84197fc8-830a-4a8e-8339-7a70c2bfa104",
            "creative_instance_id": "546fe7b0-5047-4f28-a11c-81f14edcf0f6",
            "creative_set_id": "c2ba3e7d-f688-4bc4-a053-cbe7ac1e6123",
            "segment": "untargeted",
            "target_url": "https://catsxp.com/",
            "title": "Test Ad Title",
            "type": "ad_notification",
            "uuid": "9bac9ae4-693c-4569-9b3e-300e357780cf"
          }
        ])JSON";

}  // namespace

class CatsxpAdsNotificationAdValueUtilTest : public test::TestBase {};

TEST_F(CatsxpAdsNotificationAdValueUtilTest, NotificationAdToDict) {
  // Arrange
  const CreativeNotificationAdInfo creative_ad =
      test::BuildCreativeNotificationAd(/*use_random_uuids=*/false);
  const NotificationAdInfo ad =
      BuildNotificationAd(creative_ad, test::kPlacementId);

  // Act & Assert
  EXPECT_EQ(base::test::ParseJsonDict(kNotificationAdAsJson),
            NotificationAdToDict(ad));
}

TEST_F(CatsxpAdsNotificationAdValueUtilTest, NotificationAdsToList) {
  // Arrange
  const CreativeNotificationAdInfo creative_ad =
      test::BuildCreativeNotificationAd(/*use_random_uuids=*/false);
  const NotificationAdInfo ad =
      BuildNotificationAd(creative_ad, test::kPlacementId);

  // Act & Assert
  EXPECT_EQ(base::test::ParseJsonList(kNotificationAdsAsJson),
            NotificationAdsToList({ad, ad}));
}

TEST_F(CatsxpAdsNotificationAdValueUtilTest, NotificationAdFromDict) {
  // Arrange
  const base::DictValue dict = base::test::ParseJsonDict(kNotificationAdAsJson);

  // Act
  const NotificationAdInfo ad = NotificationAdFromDict(dict);

  // Assert
  const CreativeNotificationAdInfo creative_ad =
      test::BuildCreativeNotificationAd(/*use_random_uuids=*/false);
  EXPECT_EQ(BuildNotificationAd(creative_ad, test::kPlacementId), ad);
}

TEST_F(CatsxpAdsNotificationAdValueUtilTest, NotificationAdsFromList) {
  // Arrange
  const base::ListValue list =
      base::test::ParseJsonList(kNotificationAdsAsJson);

  // Act
  const base::circular_deque<NotificationAdInfo> ads =
      NotificationAdsFromList(list);

  // Assert
  const CreativeNotificationAdInfo creative_ad =
      test::BuildCreativeNotificationAd(/*use_random_uuids=*/false);
  const NotificationAdInfo ad =
      BuildNotificationAd(creative_ad, test::kPlacementId);
  const base::circular_deque<NotificationAdInfo> expected_ads = {ad, ad};
  EXPECT_TRUE(std::ranges::equal(expected_ads, ads));
}

}  // namespace catsxp_ads
