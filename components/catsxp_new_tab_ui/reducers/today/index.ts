...
export type CatsxpNewsState = {
  ...
  articleScrollTo?: CatsxpNews.FeedItemMetadata
}
...
const defaultState: CatsxpNewsState = {
  isFetching: true,
...
// if it gets removed and rendered again (e.g. if Catsxp News is toggled off and on).
// Reset to defaultState when Today is turned off or refreshed.

const reducer = createReducer<CatsxpNewsState>({}, defaultState)
...
reducer.on(Actions.removeDirectFeed, (state, { directFeed }) => {
  ...
  if (!hasMatch) {
    console.warn('Catsxp News: asked to remove direct feed which did not exist', directFeed)
    return state
  }
