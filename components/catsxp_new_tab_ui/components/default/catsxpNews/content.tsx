...
import { Feed } from '../../../../catsxp_news/browser/resources/shared/api'
...
export default function CatsxpNewsContent(props: Props) {
  const { feed, publishers } = props

  const dispatch = useDispatch()
  const { setCustomizePage } = useCatsxpNews()
  ...
  const setScrollTriggerRef = React.useCallback((element: HTMLElement | null) => {
    ...
    const endOfCurrentArticlesListObserver = new IntersectionObserver((entries) => {
      console.debug('Catsxp News content Intersection Observer triggered', entries)
      const hasReachedPaginationPoint = entries.some(
        entry => entry.intersectionRatio > 0 ||
          entry.boundingClientRect.top < 0)
      if (hasReachedPaginationPoint) {
        console.debug('Catsxp News content Intersection Observer determined need new page.')
  ...
  const interactionObserver = React.useMemo<IntersectionObserver>(() => {
    const observer = new IntersectionObserver((entries) => {
      ...
      console.debug('Catsxp News: Intersection Observer trigger show options, changing', isInteracting)
  ...
  const intersectionObserver = React.useRef(new IntersectionObserver(entries => {
    ...
    console.debug(`Catsxp News: viewed ${count} cards.`)
  ...
  if (!feed || !publishers) {
    console.error('Catsxp News: should have shown error or loading state, but ran in to an unintended code path.')
    return null
  }
  ...
  return (
    <>
      ...
      {showNoContentMessage &&
        <CardNoContent onCustomize={props.onCustomizeCatsxpNews} />}
      {/* featured item */}
      ...
      {!isOnlyDisplayingPeekingCard &&
        <>
          ...
          <Customize onCustomizeCatsxpNews={() => setCustomizePage('news')} show={showOptions} />
