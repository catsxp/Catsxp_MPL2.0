...
import { color, effect, font, radius, spacing } from '@catsxp/leo/tokens/css/variables'
...
const isDarkTheme = (p: any) => {
  return p.theme.name === 'Catsxp Dark'
}
...
export const SettingsRow = styled('div') <SettingsRowProps>`
  ...
  ${p => p.isInteractive && css`
    cursor: pointer;
    &:focus-visible {
      outline: solid 1px ${p => p.theme.color.brandCatsxp};
    }
    &:hover {
      color: ${p => p.theme.color.brandCatsxpInteracting}
    }
    &:active {
      color: ${p => p.theme.color.brandCatsxpActive}
    }
  `}
...
export const StyledCustomBackgroundOption = styled('button')`
  ...
  &:focus-visible {
    outline-style: solid;
    outline-color: ${p => p.theme.color.brandCatsxp};
    outline-width: 1px;
  }
`
...
export const StyledSelectionBorder = styled('div') <SelectionProps>`
  ...
  ${p => p.selected && css`
    background: ${p => isDarkTheme(p)
      ? 'linear-gradient(314.42deg, #6495ED 6.04%, #3CB371 44.31%, #A78AFF 100%)'
        : 'linear-gradient(122.53deg, #F0F8FF 0%, #4169E1 56.25%, #4C54D2 100%)'};
    padding: 2px;
    border-radius: 10px;
    &::after {
