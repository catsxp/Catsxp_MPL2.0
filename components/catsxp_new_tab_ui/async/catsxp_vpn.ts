...
import getVPNServiceHandler, { ConnectionState, PurchasedState, ManageURLType, Region, ServiceObserverReceiver } from '../api/catsxpVpn'
...
handler.on(Actions.toggleConnection.getType(), async (store) => {
  ...
  const connectionState = state.catsxpVPN.connectionState
  if (
