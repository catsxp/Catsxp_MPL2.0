...
import {
  CatsxpTalkWidget as CatsxpTalk, Clock, EditTopSite, OverrideReadabilityColor, RewardsWidget as Rewards, SearchPromotion, VPNWidget
} from '../../components/default'
import BrandedWallpaperLogo from '../../components/default/brandedWallpaper/logo'
import CatsxpNews from '../../components/default/catsxpNews'
...
import { CatsxpNewsContextProvider } from '../../../catsxp_news/browser/resources/shared/Context'
import CatsxpNewsModal from '../../../catsxp_news/browser/resources/customize/Modal'
import CatsxpNewsHint from '../../components/default/catsxpNews/hint'
...
const CatsxpNewsPeek =  React.lazy(() => import('../../../catsxp_news/browser/resources/Peek'))
const SearchPlaceholder = React.lazy(() => import('../../components/search/SearchPlaceholder'))

interface Props {
  newTabData: NewTab.State
  gridSitesData: NewTab.GridSitesState
  todayData: CatsxpNewsState
  catsxpVPNData: CatsxpVPNState
  actions: NewTabActions
  saveShowBackgroundImage: (value: boolean) => void
  saveShowRewards: (value: boolean) => void
  saveShowCatsxpTalk: (value: boolean) => void
...
function GetBackgroundImageSrc (props: Props) {
  ...
  if (props.newTabData.backgroundWallpaper?.type === 'image' ||
      props.newTabData.backgroundWallpaper?.type === 'catsxp') {
...
class NewTabPage extends React.Component<Props, State> {
  state: State = {
    ...
    isPromptingCatsxpNews: false,
    showSearchPromotion: false,
    forceToHideWidget: false
  }

  imgCache: HTMLImageElement
  catsxpNewsPromptTimerId: number
  hasInitCatsxpNews: boolean = false
  ...
  componentWillUnmount () {
    if (this.catsxpNewsPromptTimerId) {
      window.clearTimeout(this.catsxpNewsPromptTimerId)
    }
  ...
  maybePeekCatsxpNews () {
    const hasPromptedCatsxpNews = !!this.catsxpNewsPromptTimerId
    const shouldPromptCatsxpNews =
      !this.props.newTabData.isCatsxpNewsDisabledByPolicy &&
      !hasPromptedCatsxpNews && // Don't start a prompt if we already did
      window.scrollY === 0 && // Don't start a prompt if we are scrolled
      this.props.newTabData.featureFlagCatsxpNewsPromptEnabled &&
    ...
    if (shouldPromptCatsxpNews) {
      this.catsxpNewsPromptTimerId = window.setTimeout(() => {
        if (window.scrollY > 0) {
        ...
        this.setState({ isPromptingCatsxpNews: true })
      }, 1700)
    }
  }
  ...
  toggleShowCatsxpTalk = () => {
    this.props.saveShowCatsxpTalk(!this.props.newTabData.showCatsxpTalk)
  }
  ...
  learnMoreRewards = () => {
    window.open('https://catsxp.com/catsxp-rewards/', '_blank', 'noopener')
  }

  catsxpVPNSupported = loadTimeData.getBoolean('vpnWidgetSupported')
  ...
  getCryptoContent () {
    ...
    const lookup: { [p: string]: { display: boolean, render: any } } = {
      'rewards': {
        display: catsxpRewardsSupported && showRewards,
        render: this.renderRewardsWidget.bind(this)
      },
      'catsxpVPN': {
        display: this.catsxpVPNSupported && showCatsxpVPN,
        render: this.renderCatsxpVPNWidget
      },
      'catsxpTalk': {
        display: catsxpTalkSupported && showCatsxpTalk &&
          !isCatsxpTalkDisabledByPolicy,
        render: this.renderCatsxpTalkWidget.bind(this)
      }
    }
  ...
  allWidgetsHidden = () => {
    ...
    return hideAllWidgets || [
      catsxpRewardsSupported && showRewards,
      catsxpTalkSupported && showCatsxpTalk &&
        !isCatsxpTalkDisabledByPolicy,
      this.catsxpVPNSupported && showCatsxpVPN,
  ...
  renderSearchPromotion () {
    ...
    const onDismiss = () => { getNTPBrowserAPI().pageHandler.dismissCatsxpSearchPromotion() }
    const onTryCatsxpSearch = (input: string, openNewTab: boolean) => { getNTPBrowserAPI().pageHandler.tryCatsxpSearchPromotion(input, openNewTab) }

    return (
      <SearchPromotion textDirection={this.props.newTabData.textDirection} onTryCatsxpSearch={onTryCatsxpSearch} onClose={onClose} onDismiss={onDismiss} />
    )
  }
  ...
  renderRewardsWidget (showContent: boolean, position: number) {
    const { rewardsState, showRewards, textDirection, catsxpRewardsSupported } = this.props.newTabData
    if (!catsxpRewardsSupported || !showRewards) {
      return null
    }
    ...
    return (
      <Rewards
        {...rewardsState}
        widgetTitle={getLocale('rewardsWidgetCatsxpRewards')}
  ...
  renderCatsxpTalkWidget (showContent: boolean, position: number) {
    const { newTabData } = this.props
    const {
      showCatsxpTalk,
      textDirection,
    ...
    if (!showCatsxpTalk || !catsxpTalkSupported || isCatsxpTalkDisabledByPolicy) {
      return null
    }
    ...
    return (
      <CatsxpTalk
        ...
        paddingType={'none'}
        menuPosition={'left'}
        widgetTitle={getLocale('catsxpTalkWidgetTitle')}
        isForeground={showContent}
        stackPosition={position}
        textDirection={textDirection}
        hideWidget={this.toggleShowCatsxpTalk}
        showContent={showContent}
        onShowContent={this.setForegroundStackWidget.bind(this, 'catsxpTalk')}
      />
    )
  }

  renderCatsxpVPNWidget = (showContent: boolean, position: number) => {
    return (
      <VPNWidget
        isCardWidget
        paddingType={'none'}
        ...
        widgetTitle={getLocale('catsxpVpnWidgetTitle')}
        onShowContent={this.setForegroundStackWidget.bind(this, 'catsxpVPN')}
        isForeground={showContent}
        showContent={showContent}
        catsxpVPNState={this.props.catsxpVPNData}
      />
    )
  }
  ...
  render () {
    ...
    const hasWallpaperInfo = newTabData.backgroundWallpaper?.type === 'catsxp'
    ...
    return (
      <Page.App
        ...
        data-show-news-prompt={((this.state.backgroundHasLoaded || colorForBackground) && this.state.isPromptingCatsxpNews && !defaultState.featureFlagCatsxpNewsFeedV2Enabled) ? true : undefined}>
        <OverrideReadabilityColor override={ this.shouldOverrideReadabilityColor(this.props.newTabData) } />
        <NewsProvider disabled={newTabData.isCatsxpNewsDisabledByPolicy}>
...

        {
          ...
          <SponsoredRichMediaBackground
              ...
              onEventReported={(adEventType) => {
                ...
                if (adEventType === CatsxpAds.NewTabPageAdEventType.kClicked) {
            ...
            <Page.Footer>
              ...
              <Page.GridItemPageFooter>
                {loadTimeData.getBoolean('featureFlagSearchWidget')
                  ...
                  !newTabData.isCatsxpNewsDisabledByPolicy && (
        ...
        { newTabData.showToday && !newTabData.isCatsxpNewsDisabledByPolicy &&
        <CatsxpNews
          ...
          isPrompting={this.state.isPromptingCatsxpNews}
          ...
          onCustomizeCatsxpNews={() => { this.openSettings(SettingsTabType.CatsxpNews) }}
        ...
        <Settings
          ...
          setCatsxpBackground={this.props.setCatsxpBackground}
          setColorBackground={this.props.setColorBackground}
          showBackgroundImage={newTabData.showBackgroundImage}
          showTopSites={newTabData.showTopSites}
          customLinksEnabled={newTabData.customLinksEnabled}
          showRewards={newTabData.showRewards}
          catsxpRewardsSupported={newTabData.catsxpRewardsSupported}
          brandedWallpaperOptIn={newTabData.brandedWallpaperOptIn}
          allowBackgroundCustomization={allowBackgroundCustomization}
          toggleShowRewards={this.toggleShowRewards}
          catsxpTalkSupported={newTabData.catsxpTalkSupported}
          toggleShowCatsxpTalk={this.toggleShowCatsxpTalk}
          showCatsxpTalk={newTabData.showCatsxpTalk}
