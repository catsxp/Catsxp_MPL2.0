...
function getWidgetStackOrder(firstWidget: string): NewTab.StackWidget[] {
  switch (firstWidget) {
    case 'catsxpTalk':
      return ['rewards', 'catsxpTalk']
    default:
      return ['catsxpTalk', 'rewards']
  }
}
...
export const useNewTabData = (state: NewTab.State = defaultState) => {
  const result: NewTab.State = {
    ...
    featureFlagCatsxpNTPSponsoredImagesWallpaper: true,
    featureCustomBackgroundEnabled: true,
    featureFlagCatsxpNewsPromptEnabled: true,
    featureFlagCatsxpNewsFeedV2Enabled: false,
    ...
    catsxpTalkSupported: boolean('Catsxp Talk supported?', true),
    hideAllWidgets: boolean('Hide all widgets?', false),
    isCatsxpNewsOptedIn: boolean('Catsxp News opted-in?', false),
    ...
    widgetStackOrder: getWidgetStackOrder(select('First widget', ['catsxpTalk', 'rewards'], 'rewards'))
  }
