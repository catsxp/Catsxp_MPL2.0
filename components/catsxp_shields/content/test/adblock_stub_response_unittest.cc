...
#include "catsxp/components/catsxp_shields/content/browser/adblock_stub_response.h"
...
TEST(AdBlockStubResponse, ScriptDataURL) {
  ...
  catsxp_shields::MakeStubResponse(data_url, request, &resource_response, &data);
...
TEST(AdBlockStubResponse, AcceptHeaderUsedNoDataURL) {
  ...
  auto resource_response = network::mojom::URLResponseHead::New();
  catsxp_shields::MakeStubResponse("", request, &resource_response, &data);
  ASSERT_EQ(data, "");
  ASSERT_EQ(resource_response->mime_type, "text/xml");
}
