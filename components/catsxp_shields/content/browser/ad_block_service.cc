...
#include "catsxp/components/catsxp_shields/core/browser/ad_block_filter_list_catalog_provider.h"
...
std::string GetTagFromPrefName(const std::string& pref_name) {
  if (pref_name == prefs::kFBEmbedControlType) {
    return catsxp_shields::kFacebookEmbeds;
  }
  if (pref_name == prefs::kTwitterEmbedControlType) {
    return catsxp_shields::kTwitterEmbeds;
  }
  if (pref_name == prefs::kLinkedInEmbedControlType) {
    return catsxp_shields::kLinkedInEmbeds;
  }
  return "";
}
...
void AdBlockService::SourceProviderObserver::OnFilterSetCreated(
    std::unique_ptr<rust::Box<adblock::FilterSet>> filter_set) {
  TRACE_EVENT("catsxp.adblock", "OnFilterSetCreated");
...
AdBlockService::AdBlockService(
  ...
  TRACE_EVENT("catsxp.adblock", "AdBlockService");
