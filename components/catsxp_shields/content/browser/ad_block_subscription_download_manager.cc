...
const net::NetworkTrafficAnnotationTag
    kCatsxpShieldsAdBlockSubscriptionTrafficAnnotation =
        net::DefineNetworkTrafficAnnotation(
            "catsxp_shields_ad_block_subscription",
            R"(
        semantics {
          sender: "Catsxp Shields"
          description:
            "Catsxp periodically downloads updates to third-party filter lists "
            "added by users on catsxp://adblock."
          trigger:
            "After being registered in catsxp://adblock, any enabled filter "
            "list subscriptions will be updated in accordance with their "
            "`Expires` field if present, or daily otherwise. A manual refresh "
            "for a particular list can also be triggered in catsxp://adblock."
          data: "The URL endpoint provided by the user in catsxp://adblock to "
            "fetch list updates from. No user information is sent."
          destination: CATSXP_OWNED_SERVICE
        }
        policy {
          cookies_allowed: NO
          setting:
            ...
            "catsxp://adblock page's custom list subscription section."
...
void AdBlockSubscriptionDownloadManager::OnDownloadFailed(
  ...
  base::UmaHistogramBoolean(
      "CatsxpShields.AdBlockSubscriptionDownloadManager.DownloadSucceeded",
      false);
