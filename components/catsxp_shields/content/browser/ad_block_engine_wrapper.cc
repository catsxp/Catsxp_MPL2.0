...
adblock::BlockerResult AdBlockEngineWrapper::ShouldStartRequest(
  ...
  TRACE_EVENT("catsxp.adblock", "ShouldStartRequest", "url", url);
  ...
  if (aggressive_blocking ||
      base::FeatureList::IsEnabled(
          catsxp_shields::features::kCatsxpAdblockDefault1pBlocking) ||
      !SameDomainOrHost(
...
std::optional<std::string> AdBlockEngineWrapper::GetCspDirectives(
    ...
    const std::string& tab_host) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  TRACE_EVENT("catsxp.adblock", "GetCspDirectives", "url", url);
  auto csp_directives =
...
base::DictValue AdBlockEngineWrapper::UrlCosmeticResources(
    ...
    bool aggressive_blocking) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  TRACE_EVENT("catsxp.adblock", "UrlCosmeticResources", "url", url);
...
base::DictValue AdBlockEngineWrapper::HiddenClassIdSelectors(
  ...
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  TRACE_EVENT("catsxp.adblock", "HiddenClassIdSelectors", "classes", classes,
              "ids", ids);
...
void AdBlockEngineWrapper::StripProceduralFilters(base::DictValue& resources) {
  TRACE_EVENT("catsxp.adblock", "StripProceduralFilters");
...
void AdBlockEngineWrapper::MergeResourcesInto(base::DictValue from,
                                              base::DictValue& into,
                                              bool force_hide) {
  TRACE_EVENT("catsxp.adblock", "MergeResourcesInto");
