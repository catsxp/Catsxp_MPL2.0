...
std::string_view GetContentForMimeType(std::string_view mime_type) {
  static constexpr auto kContent =
      base::MakeFixedFlatMap<std::string_view, base::span<const uint8_t>>({
          {"image/avif", base::span(kAvif1x1)},
          {"image/webp", base::span(kWebp1x1)},
          {"image/*", base::span(kPng1x1)},
          {"image/apng", base::span(kPng1x1)},
          {"image/png", base::span(kPng1x1)},
          {"image/x-png", base::span(kPng1x1)},
          {"image/gif", base::span(kGif1x1)},
          {"image/jpeg", base::span(kJpeg1x1)},
      });
  auto it = kContent.find(mime_type);
  if (it == kContent.end()) {
    return {};
  }
  return base::as_string_view(it->second);
}
