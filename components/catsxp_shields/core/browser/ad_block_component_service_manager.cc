...
#include "catsxp/components/catsxp_shields/core/browser/ad_block_list_p3a.h"
...
using catsxp_shields::features::kCatsxpAdblockCookieListDefault;
using catsxp_shields::features::kCatsxpAdblockExperimentalListDefault;
using catsxp_shields::features::kCatsxpAdblockMobileNotificationsListDefault;
...
bool IsAdBlockOnlyModeSupportedAndFeatureEnabled(const std::string& locale) {
  return base::FeatureList::IsEnabled(
         ...
         catsxp_shields::IsAdblockOnlyModeSupportedForLocale(locale);
}
...
void AdBlockComponentServiceManager::StartRegionalServices() {
  ...
  if (!checked_all_default_regions) {
    ...
    auto locale_lists = catsxp_shields::FindAdBlockFilterListsByLocale(
...
bool AdBlockComponentServiceManager::IsFilterListAvailable(
  ...
  DCHECK(!uuid.empty());
  auto catalog_entry =
      catsxp_shields::FindAdBlockFilterListByUUID(filter_list_catalog_, uuid);
...
void AdBlockComponentServiceManager::UpdateFilterLists(
  ...
  catsxp_component_updater::CatsxpOnDemandUpdater::GetInstance()->OnDemandUpdate(
...
base::ListValue AdBlockComponentServiceManager::GetRegionalLists() {
  ...
  for (const auto& region_list : filter_list_catalog_) {
    ...
    // https://github.com/catsxp/adblock-resources
    base::DictValue dict;
