...
#include "catsxp/components/p3a/utils.h"
...
ShieldsSettingCounts GetAdsSettingCount(HostContentSettingsMap* map) {
  ContentSettingsForOneType cosmetic_rules =
      map->GetSettingsForOneType(ContentSettingsType::CATSXP_COSMETIC_FILTERING);
...
void ReportManualShredP3A(const PrefService& local_state) {
  if (!base::FeatureList::IsEnabled(features::kCatsxpShredFeature)) {
    return;
  }
...
void RecordShieldsDomainSettingCounts(PrefService* profile_prefs,
  ...
  VLOG(1) << "CatsxpShieldsP3A: Recording counts: is_fp=" << is_fingerprint
...
void RecordShieldsDomainSettingCountsWithChange(PrefService* profile_prefs,
  ...
  if (prev_setting != nullptr) {
    ...
    VLOG(1) << "CatsxpShieldsP3A: Decreasing prev setting count: prev_setting="
  ...
  VLOG(1) << "CatsxpShieldsP3A: Increasing new setting count: new_setting="
...
void RecordHTTPSUpgradeSettingP3A(HostContentSettingsMap* map) {
  ContentSetting global_setting = map->GetContentSetting(
      GURL(), GURL(), ContentSettingsType::CATSXP_HTTPS_UPGRADE);
  ...
  auto per_site_settings =
      map->GetSettingsForOneType(ContentSettingsType::CATSXP_HTTPS_UPGRADE);
...
void MaybeRecordInitialShieldsSettings(
  ...
  VLOG(1) << "CatsxpShieldsP3A: Starting initial report for profile";
  ...
  VLOG(1) << "CatsxpShieldsP3A: Domain FP counts: allow=" << fp_counts.allow
          << " standard=" << fp_counts.standard
          << " agg=" << fp_counts.aggressive;
  VLOG(1) << "CatsxpShieldsP3A: Domain Ad counts: allow=" << ads_counts.allow
