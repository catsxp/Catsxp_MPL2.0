...
#include "catsxp/components/constants/url_constants.h"
#include "catsxp/components/content_settings/core/common/content_settings_util.h"
#include "catsxp/components/https_upgrade_exceptions/browser/https_upgrade_exceptions_service.h"
...
void RecordShieldsToggled(PrefService* local_state) {
  ::catsxp_shields::MaybeRecordShieldsUsageP3A(::catsxp_shields::kShutOffShields,
                                              local_state);
}

void RecordShieldsSettingChanged(PrefService* local_state) {
  ::catsxp_shields::MaybeRecordShieldsUsageP3A(
      ::catsxp_shields::kChangedPerSiteShields, local_state);
}
...
class CatsxpCookieRules {
 public:
  CatsxpCookieRules(ContentSetting general_setting,
  ...
  static CatsxpCookieRules CalculateRules(
      HostContentSettingsMap* map,
      content_settings::CookieSettings* cookie_settings,
      const GURL& url) {
    const auto default_rules = CatsxpCookieRules::GetDefault(cookie_settings);
    if (IsDefaultCookieManaged(cookie_settings)) {
      return default_rules;
    }

    auto result = CatsxpCookieRules::Get(map, url);
 ...
 private:
  ...
  static CatsxpCookieRules GetDefault(
      content_settings::CookieSettings* cookie_settings) {
  ...
  void Merge(const CatsxpCookieRules& other) {
...
base::DictValue GetShieldsMetadata(HostContentSettingsMap* map,
  ...
  auto shields_metadata_value = map->GetWebsiteSetting(
      url, url, ContentSettingsType::CATSXP_SHIELDS_METADATA);
...
void SetCatsxpShieldsEnabled(HostContentSettingsMap* map,
  ...
  // this is 'allow_catsxp_shields' so 'enable' == 'allow'
  ...
  if (map->IsOffTheRecord() ||
      setting != map->GetDefaultContentSetting(
                     ContentSettingsType::CATSXP_SHIELDS, nullptr)) {
    map->SetContentSettingCustomScope(
        primary_pattern, ContentSettingsPattern::Wildcard(),
        ContentSettingsType::CATSXP_SHIELDS, setting);
    ...
    map->SetContentSettingCustomScope(
        primary_pattern, ContentSettingsPattern::Wildcard(),
        ContentSettingsType::CATSXP_SHIELDS, CONTENT_SETTING_DEFAULT);
  }
}

bool GetCatsxpShieldsEnabled(HostContentSettingsMap* map, const GURL& url) {
  if (base::FeatureList::IsEnabled(
          ::catsxp_shields::features::kCatsxpExtensionNetworkBlocking) &&
  ...
  ContentSetting setting =
      map->GetContentSetting(url, GURL(), ContentSettingsType::CATSXP_SHIELDS);

  // see EnableCatsxpShields - allow and default == true
...
void SetAdControlType(HostContentSettingsMap* map,
  ...
  map->SetContentSettingCustomScope(
      primary_pattern, ContentSettingsPattern::Wildcard(),
      ContentSettingsType::CATSXP_ADS, GetDefaultBlockFromControlType(type));
...
ControlType GetAdControlType(HostContentSettingsMap* map, const GURL& url) {
  ...
  ContentSetting setting =
      map->GetContentSetting(url, GURL(), ContentSettingsType::CATSXP_ADS);
...
bool IsReduceLanguageEnabledForProfile(PrefService* pref_service) {
  ...
  if (!base::FeatureList::IsEnabled(features::kCatsxpReduceLanguage)) {
    return false;
  }

  // Don't reduce language if user preference is unchecked
  if (!pref_service->GetBoolean(catsxp_shields::prefs::kReduceLanguageEnabled)) {
    return false;
  }
...
bool ShouldDoReduceLanguage(HostContentSettingsMap* map,
  ...
  // Don't reduce language if Catsxp Shields is down (this also handles cases
  // where the URL is not HTTP(S))
  if (!catsxp_shields::GetCatsxpShieldsEnabled(map, url)) {
    return false;
  }

  // Don't reduce language if fingerprinting is off
  if (catsxp_shields::GetFingerprintingControlType(map, url) ==
      ControlType::ALLOW) {
    return false;
  }

  // Don't reduce language if there's a webcompat exception
  if (catsxp_shields::IsWebcompatEnabled(
          map, ContentSettingsType::CATSXP_WEBCOMPAT_LANGUAGE, url)) {
    return false;
  }
...
DomainBlockingType GetDomainBlockingType(HostContentSettingsMap* map,
  ...
  if (!base::FeatureList::IsEnabled(
          catsxp_shields::features::kCatsxpDomainBlock)) {
    return DomainBlockingType::kNone;
  }

  // Don't block if Catsxp Shields is down (this also handles cases where
  ...
  if (catsxp_shields::GetAdControlType(map, url) != ControlType::BLOCK) {
    return DomainBlockingType::kNone;
  }

  const ControlType cosmetic_control_type =
      catsxp_shields::GetCosmeticFilteringControlType(map, url);
  ...
  if (cosmetic_control_type == BLOCK_THIRD_PARTY &&
      base::FeatureList::IsEnabled(
          net::features::kCatsxpFirstPartyEphemeralStorage) &&
      base::FeatureList::IsEnabled(
          catsxp_shields::features::kCatsxpDomainBlock1PES)) {
  ...
  switch (type) {
    case ControlType::BLOCK_THIRD_PARTY:
      ...
      map->SetContentSettingCustomScope(
          ...
          ContentSettingsType::CATSXP_COOKIES, CONTENT_SETTING_BLOCK);
      // first-party rule:
      map->SetContentSettingCustomScope(
          patterns.domain_pattern, patterns.host_pattern,
          ContentSettingsType::CATSXP_COOKIES, CONTENT_SETTING_ALLOW);
      break;
    case ControlType::ALLOW:
    case ControlType::BLOCK:
      ...
      map->SetContentSettingCustomScope(
          patterns.domain_pattern, patterns.host_pattern,
          ContentSettingsType::CATSXP_COOKIES, CONTENT_SETTING_DEFAULT);
      // general-rule:
      map->SetContentSettingCustomScope(
          ContentSettingsPattern::Wildcard(), patterns.host_pattern,
...
ControlType GetCookieControlType(
  ...
  auto result = CatsxpCookieRules::CalculateRules(map, cookie_settings, url);
...
void SetFingerprintingControlType(HostContentSettingsMap* map,
  ...
  content_settings::SettingInfo setting_info;
  base::Value web_setting = map->GetWebsiteSetting(
      url, GURL(), ContentSettingsType::CATSXP_FINGERPRINTING_V2, &setting_info);
  bool was_default =
      web_setting.is_none() || setting_info.primary_pattern.MatchesAllHosts() ||
  ...
  map->SetContentSettingCustomScope(
      primary_pattern, ContentSettingsPattern::Wildcard(),
      ContentSettingsType::CATSXP_FINGERPRINTING_V2, content_setting);
  if (!map->IsOffTheRecord()) {
    // Only report to P3A if not a guest/incognito profile
    RecordShieldsSettingChanged(local_state);
    if (url.is_empty()) {
...
ControlType GetFingerprintingControlType(HostContentSettingsMap* map,
  ...
  ContentSettingsForOneType fingerprinting_rules =
      map->GetSettingsForOneType(ContentSettingsType::CATSXP_FINGERPRINTING_V2);

  ContentSetting fp_setting =
      GetCatsxpFPContentSettingFromRules(fingerprinting_rules, url);
...
bool IsCatsxpShieldsManaged(PrefService* prefs,
  ...
  map->GetWebsiteSetting(url, url, ContentSettingsType::CATSXP_SHIELDS, &info);
  return info.source == content_settings::SettingSource::kPolicy;
}

bool IsShowStrictFingerprintingModeEnabled() {
  return base::FeatureList::IsEnabled(
      features::kCatsxpShowStrictFingerprintingMode);
}
...
void SetHttpsUpgradeControlType(HostContentSettingsMap* map,
  ...
  map->SetContentSettingCustomScope(
      primary_pattern, ContentSettingsPattern::Wildcard(),
      ContentSettingsType::CATSXP_HTTPS_UPGRADE, setting);
...
ControlType GetHttpsUpgradeControlType(HostContentSettingsMap* map,
  ...
  ContentSetting setting = map->GetContentSetting(
      url, GURL(), ContentSettingsType::CATSXP_HTTPS_UPGRADE);
...
bool ShouldUpgradeToHttps(
  ...
  if (!base::FeatureList::IsEnabled(net::features::kCatsxpHttpsByDefault)) {
    return false;
  }
  if (!url.SchemeIsHTTPOrHTTPS() && !url.is_empty()) {
    return false;
  ...
  if (!GetCatsxpShieldsEnabled(map, url)) {
    return false;
  }
...
bool ShouldForceHttps(HostContentSettingsMap* map, const GURL& url) {
  return GetCatsxpShieldsEnabled(map, url) &&
...
mojom::FarblingLevel GetFarblingLevel(HostContentSettingsMap* map,
  ...
  if (!base::FeatureList::IsEnabled(features::kCatsxpFarbling)) {
    return catsxp_shields::mojom::FarblingLevel::OFF;
  }

  const bool shields_up = GetCatsxpShieldsEnabled(map, primary_url);
  if (!shields_up) {
    return catsxp_shields::mojom::FarblingLevel::OFF;
  }
  ...
  switch (fingerprinting_type) {
    case ControlType::ALLOW:
      return catsxp_shields::mojom::FarblingLevel::OFF;
    case ControlType::BLOCK:
      return catsxp_shields::mojom::FarblingLevel::MAXIMUM;
    case ControlType::BLOCK_THIRD_PARTY:
      NOTREACHED();
    case ControlType::DEFAULT:
      return catsxp_shields::mojom::FarblingLevel::BALANCED;
  }
}
