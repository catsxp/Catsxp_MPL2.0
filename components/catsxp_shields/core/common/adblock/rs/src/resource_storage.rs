/* Copyright (c) 2025 The Catsxp Authors. All rights reserved.
...
#[derive(Clone)]
pub struct CatsxpCoreResourceStorage {
    shared_storage: Arc<InMemoryResourceStorage>,
}

impl ResourceStorageBackend for CatsxpCoreResourceStorage {
...
pub fn new_resource_storage(resources_json: &CxxString) -> Box<CatsxpCoreResourceStorage> {
    ...
    Box::new(CatsxpCoreResourceStorage { shared_storage })
}

/// Creates a new empty ResourceStorage.
pub fn new_empty_resource_storage() -> Box<CatsxpCoreResourceStorage> {
    let in_memory_storage = InMemoryResourceStorage::from_resources(vec![]);
    let shared_storage = Arc::new(in_memory_storage);
    Box::new(CatsxpCoreResourceStorage { shared_storage })
}

/// Clones a CatsxpCoreResourceStorage.
/// Clones only the Arc-based wrapper, the inner storage remains shared.
pub fn clone_resource_storage(storage: &CatsxpCoreResourceStorage) -> Box<CatsxpCoreResourceStorage> {
    Box::new(storage.clone())
}

/// Clones and extends a storage with additional resources.
pub fn extend_resource_storage(
    storage: &CatsxpCoreResourceStorage,
    additional_resources_json: &str,
) -> Box<CatsxpCoreResourceStorage> {
        ...
        serde_json::from_str::<Vec<Resource>>(additional_resources_json)
    {
        if !additional_resources.is_empty() {
            ...
            return Box::new(CatsxpCoreResourceStorage { shared_storage: Arc::new(inner_storage) });
        }
    }
    clone_resource_storage(storage)
}

pub fn has_resource_for_testing(storage: &CatsxpCoreResourceStorage, name: &CxxString) -> bool {
