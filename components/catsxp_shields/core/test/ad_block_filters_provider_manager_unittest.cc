...
class FiltersProviderManagerTestObserver
    : public catsxp_shields::AdBlockFiltersProvider::Observer {
 public:
...
TEST(AdBlockFiltersProviderManagerTest, MaybeNotifyObserverNotifiesWhenReady) {
  catsxp_shields::AdBlockFiltersProviderManager m;

  catsxp_shields::TestFiltersProvider provider1("rules_a", true, 0);
...
TEST(AdBlockFiltersProviderManagerTest, ForceNotifyObserverRespectsEngineType) {
  ...
  catsxp_shields::TestFiltersProvider default_provider("default_rules", true, 0);
  default_provider.RegisterAsSourceProvider(&m);

  catsxp_shields::TestFiltersProvider additional_provider("additional_rules",
                                                         false, 0);
...
TEST(AdBlockFiltersProviderManagerTest,
  ...
  catsxp_shields::TestFiltersProvider provider2("rules_b", true, 0);
  ...
  void SetUp() override {
    if (GetParam()) {
      feature_list_.InitAndEnableFeature(
          catsxp_shields::features::kAdblockDATCache);
    } else {
...
TEST_P(AdBlockFiltersProviderManagerDATCacheTest, OnChangedNotifiesWhenReady) {
  ...
  catsxp_shields::TestFiltersProvider provider1("", true, 0);
  EXPECT_EQ(observer.changed_count, 0);
  provider1.RegisterAsSourceProvider(&m);
  EXPECT_EQ(observer.changed_count, dat_cache_enabled ? 0 : 1);

  catsxp_shields::TestFiltersProvider provider2("", true, 0);
...
TEST_P(AdBlockFiltersProviderManagerDATCacheTest,
  ...
  catsxp_shields::TestFiltersProvider provider("rules", true, 0);
