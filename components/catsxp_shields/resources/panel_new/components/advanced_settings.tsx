...
function AdBlockControl(props: { showDetails: () => void }) {
  ...
  const isManaged = siteBlockInfo.isCatsxpShieldsManaged
  ...
  return (
    <div>
      ...
      <Button
        ...
        kind='plain-faint'
        fab
        aria-label={getString('CATSXP_SHIELDS_VIEW_DETAILS')}
      >
...
function getAdBlockModeLabel(mode: number | undefined) {
  switch (mode) {
    case AdBlockMode.AGGRESSIVE:
      return 'CATSXP_SHIELDS_TRACKERS_AND_ADS_BLOCKED_AGG'
    case AdBlockMode.STANDARD:
      return 'CATSXP_SHIELDS_TRACKERS_AND_ADS_BLOCKED_STD'
    case AdBlockMode.ALLOW:
      return 'CATSXP_SHIELDS_TRACKERS_AND_ADS_ALLOW_ALL'
  }
    ...
    <div>
      ...
      >
        <leo-option value={String(HttpsUpgradeMode.STRICT_MODE)}>
          {getString('CATSXP_SHIELDS_HTTPS_UPGRADE_MODE_STRICT')}
        </leo-option>
        <leo-option value={String(HttpsUpgradeMode.STANDARD_MODE)}>
          {getString('CATSXP_SHIELDS_HTTPS_UPGRADE_MODE_STANDARD')}
        </leo-option>
        <leo-option value={String(HttpsUpgradeMode.DISABLED_MODE)}>
          {getString('CATSXP_SHIELDS_HTTPS_UPGRADE_MODE_DISABLED')}
        </leo-option>
      </Dropdown>
      <div />
    </div>
    ...
    <div>
      ...
      <div className='toggle'>
        <div className='block-scripts-text'>
          <div>
            {getString('CATSXP_SHIELDS_SCRIPTS_BLOCKED')}
...
function getEnforcedDescription(overrideSource: number | undefined): StringKey {
  switch (overrideSource) {
    case ContentSettingSource.kExtension:
      return 'CATSXP_SHIELDS_SCRIPTS_BLOCKED_OVERRIDDEN_BY_EXTENSION'
    case ContentSettingSource.kPolicy:
      return 'CATSXP_SHIELDS_SCRIPTS_BLOCKED_OVERRIDDEN_BY_POLICY'
    case ContentSettingSource.kAllowList:
      return 'CATSXP_SHIELDS_SCRIPTS_BLOCKED_OVERRIDDEN_BY_ALLOWLIST'
    case ContentSettingSource.kSupervised:
      return 'CATSXP_SHIELDS_SCRIPTS_BLOCKED_OVERRIDDEN_BY_SUPERVISOR'
    case ContentSettingSource.kInstalledWebApp:
      return 'CATSXP_SHIELDS_SCRIPTS_BLOCKED_OVERRIDDEN_BY_PWA'
    default:
      return 'CATSXP_SHIELDS_SCRIPTS_BLOCKED_OVERRIDDEN'
  }
}
      ...
      {showStrictMode ? (
        ...
        >
          <leo-option value={String(FingerprintMode.STRICT_MODE)}>
            {getString('CATSXP_SHIELDS_FINGERPRINTING_BLOCKED_AGG')}
          </leo-option>
          <leo-option value={String(FingerprintMode.STANDARD_MODE)}>
            {getString('CATSXP_SHIELDS_FINGERPRINTING_BLOCKED_STD')}
          </leo-option>
          <leo-option value={String(FingerprintMode.ALLOW_MODE)}>
            {getString('CATSXP_SHIELDS_FINGERPRINTING_ALLOW_ALL')}
          </leo-option>
        </Dropdown>
      ) : (
        <div className='toggle'>
    ...
    <div>
      ...
      >
        <leo-option value={String(CookieBlockMode.BLOCKED)}>
          {getString('CATSXP_SHIELDS_COOKIES_BLOCKED')}
        </leo-option>
        <leo-option value={String(CookieBlockMode.CROSS_SITE_BLOCKED)}>
          {getString('CATSXP_SHIELDS_THIRD_PARTY_COOKIES_BLOCKED')}
        </leo-option>
        <leo-option value={String(CookieBlockMode.ALLOW)}>
          {getString('CATSXP_SHIELDS_COOKIES_ALLOWED_ALL')}
        </leo-option>
      </Dropdown>
      <div />
    </div>
...
function ForgetFirstPartyStorageControls() {
  ...
  const forgetFeatureEnabled =
    api.useIsCatsxpForgetFirstPartyStorageFeatureEnabledData()
  ...
  return (
    <div>
      <Icon name='broom' />
      <div className='toggle'>
        {getString('CATSXP_SHIELDS_FORGET_FIRST_PARTY_STORAGE_LABEL')}
        <Toggle
  ...
  return (
    <div>
      ...
      <div className='block-elements'>
        <div>{getString('CATSXP_SHIELDS_BLOCK_ELEMENTS')}</div>
        <button onClick={() => api.resetBlockedElements()}>
          {getString('CATSXP_SHIELDS_SHOW_ALL_BLOCKED_ELEMENTS')}
        </button>
      </div>
      <div />
    </div>
