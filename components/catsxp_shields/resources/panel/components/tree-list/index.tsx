...
const kLearnMoreLink = 'https://www.catsxp.com/hc/en-us/articles/360022806212-How-do-I-use-Shields-while-browsing#h_01HXSZ8JPHR8YMBEZCT5M0VZTR'
...
function generateWebcompatEntries (invokedWebcompatList: number[] | undefined) : Array<[string, number]> {
  ...
  for (const name of names) {
    ...
    if (value > ContentSettingsType.CATSXP_WEBCOMPAT_NONE &&
        value < ContentSettingsType.CATSXP_WEBCOMPAT_ALL &&
        invokedWebcompatList.includes(value)) {
      const displayName = name.replace("CATSXP_WEBCOMPAT_", "").replaceAll("_", " ").toLowerCase();
    ...
    <S.Box>
      ...
      <Footer>
        ...
        >
          ...
          {getLocale('catsxpShieldsStandalone')}
        </Button>
      </Footer>
    </S.Box>
  )
        ...
        <>
          <ScriptsInfo>
            ...
            <span>{<Link href="#" onClick={handleBlockAllScripts}>
              {getLocale('catsxpShieldsBlockScriptsAll')}
            </Link>
            }</span>
          </ScriptsInfo>
          <ScriptsList>
            {[...allowedScriptsByOrigin.keys()].map((origin, idx) => {
              return (<TreeNode
                ...
                permissionButtonTitle={getLocale('catsxpShieldsBlockScript')}
              />)
            })}
          </ScriptsList>
        </>
      ...
      <ScriptsInfo>
        ...
        {props.allowedList && (<span>
          {<Link href="#" onClick={handleAllowAllScripts}>
            {getLocale('catsxpShieldsAllowScriptsAll')}
          </Link>
          }</span>
        )}
      </ScriptsInfo>
      <ScriptsList>
        {[...blockedScriptsByOrigin.keys()].map((origin, idx) => {
          return (<TreeNode
            ...
            permissionButtonTitle={getLocale('catsxpShieldsAllowScriptOnce')}
          />)
        })}
      </ScriptsList>
    </SidePanel>
