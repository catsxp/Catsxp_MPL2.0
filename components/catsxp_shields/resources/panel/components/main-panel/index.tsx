...
import {
  AdBlockOnlyModePromptAfterRepeatedReloads,
  AdBlockOnlyModePromptAfterShieldsDisabled
} from '../ad-block-only-mode-prompt'
...
const handleLearnMoreClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
  e.preventDefault()
  chrome.tabs.create({ url: 'https://catsxp.com/privacy-features/', active: true })
}
...
function MainPanel () {
  ...
  if (urlParams.get('mode') === 'afterRepeatedReloads') {
    return <AdBlockOnlyModePromptAfterRepeatedReloads />
  }

  const catsxpShieldsStatus = formatLocale(siteBlockInfo?.isCatsxpShieldsEnabled
    ? 'catsxpShieldsUp'
    : 'catsxpShieldsDown', {
        $1: (content) => <span>{content}</span>,
    })

  const catsxpShieldsBrokenText = getLocale('catsxpShieldsBroken')
  const catsxpShieldsNoteKey = siteBlockInfo?.isCatsxpShieldsAdBlockOnlyModeEnabled
    ? 'catsxpShieldsAdsBlockedNote'
    : siteBlockInfo?.isCatsxpShieldsEnabled
      ? 'catsxpShieldsBlockedNote'
      : 'catsxpShieldsNOTBlockedNote';
  const catsxpShieldsNote = formatLocale(catsxpShieldsNoteKey, {
    $1: content => <a href='https://catsxp.com/privacy-features/'
  ...
  const handleToggleChange = async (detail: { checked: boolean }) => {
    await getPanelBrowserAPI().dataHandler.setCatsxpShieldsEnabled(detail.checked)
    if (detail.checked) {
  ...
  let reportSiteOrFootnoteElement = (
    <S.Footnote>
      {catsxpShieldsBrokenText}
      {!siteBlockInfo?.isCatsxpShieldsAdBlockOnlyModeEnabled && <>
          {' '}
          {formatLocale('catsxpShieldsBrokenNote', {
            $1: content => <span>{content}</span>
          })}
        </>
      }
  ...
  let managedFootnoteElement = (
    <S.Footnote>
      <S.ControlBox>
        <ManagedIcon />
        <S.ManagedText>{getLocale('catsxpShieldsManaged')}</S.ManagedText>
      </S.ControlBox>
    </S.Footnote>
  )
  let advancedControlButtonElement = (
    isExpanded != null && !siteBlockInfo?.isCatsxpShieldsAdBlockOnlyModeEnabled
  ) && (
    ...
    >
      ...
      <span>{getLocale('catsxpShieldsAdvancedCtrls')}</span>
  ...
  if (!siteBlockInfo?.isCatsxpShieldsEnabled) {
    ...
    advancedControlButtonElement = (
      ...
      >
        ...
        <span>{getLocale('catsxpShieldsChangeDefaults')}</span>
      </S.GlobalDefaultsButton>
    )
    ...
    reportSiteOrFootnoteElement = (
      <S.ReportSiteBox>
        <S.ReportSiteAction>
          <span>{getLocale('catsxpShieldsReportSiteDesc')}</span>
          {siteBlockInfo?.showShieldsDisabledAdBlockOnlyModePrompt ? (
              ...
              >
                {getLocale('catsxpShieldsReportSite')}
              </Button>
            ) : (
              <Button
                kind="filled"
  ...
  return (
    <S.Box>
      <S.StatusBox>
        <S.ControlBox>
          <S.ShieldsIcon isActive={siteBlockInfo?.isCatsxpShieldsEnabled ?? false}>
          ...
          <S.StatusToggle>
            <Toggle
              checked={siteBlockInfo?.isCatsxpShieldsEnabled}
              onChange={handleToggleChange}
              disabled={siteBlockInfo?.isCatsxpShieldsManaged}
            />
          </S.StatusToggle>
        </S.ControlBox>
        {!siteBlockInfo?.isCatsxpShieldsManaged &&
          !(siteBlockInfo?.isCatsxpShieldsAdBlockOnlyModeEnabled &&
            !siteBlockInfo?.isCatsxpShieldsEnabled) &&
        <S.StatusFootnoteBox>
          {reportSiteOrFootnoteElement}
      ...
      {siteBlockInfo?.isCatsxpShieldsAdBlockOnlyModeEnabled && (
        <AdBlockOnlyModeControlsContent
          isCatsxpShieldsEnabled={siteBlockInfo?.isCatsxpShieldsEnabled}
        />
      )}
      {advancedControlButtonElement}
      { isExpanded &&
      ...
      }
      {
        ...
        >
          <span>{getLocale('catsxpShieldsShowAllBlockedElems')}</span>
        </S.GlobalDefaultsButton>
      }
    </S.Box>
  )
