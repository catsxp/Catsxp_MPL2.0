...
const updateCustomFilters = debounce((customFilters: string) => {
  chrome.send('catsxp_adblock.updateCustomFilters', [customFilters])
}, 1500)
...
const adblockReducer: Reducer<AdBlock.State | undefined> = (
  ...
  switch (action.type) {
    case types.ADBLOCK_ENABLE_FILTER_LIST:
      chrome.send('catsxp_adblock.enableFilterList', [
        action.payload.uuid,
    ...
    case types.ADBLOCK_GET_CUSTOM_FILTERS:
      chrome.send('catsxp_adblock.getCustomFilters')
      break
    case types.ADBLOCK_GET_REGIONAL_LISTS:
      chrome.send('catsxp_adblock.getRegionalLists')
      break
    case types.ADBLOCK_GET_LIST_SUBSCRIPTIONS:
      chrome.send('catsxp_adblock.getListSubscriptions')
      break
    case types.ADBLOCK_SUBMIT_NEW_SUBSCRIPTION:
      chrome.send('catsxp_adblock.submitNewSubscription', [
        action.payload.listUrl,
      ])
      break
    case types.ADBLOCK_SET_SUBSCRIPTION_ENABLED:
      chrome.send('catsxp_adblock.setSubscriptionEnabled', [
        action.payload.listUrl,
        action.payload.enabled,
      ])
      break
    case types.ADBLOCK_DELETE_SUBSCRIPTION:
      chrome.send('catsxp_adblock.deleteSubscription', [action.payload.listUrl])
      break
    case types.ADBLOCK_REFRESH_SUBSCRIPTION:
      chrome.send('catsxp_adblock.refreshSubscription', [action.payload.listUrl])
      break
    case types.ADBLOCK_VIEW_SUBSCRIPTION_SOURCE:
      chrome.send('catsxp_adblock.viewSubscriptionSource', [
        action.payload.listUrl,
      ])
      break
