...
import catsxpLogoUrl
  from 'gen/catsxp/components/catsxp_welcome_ui/welcome_logo@2x.webp'
...
function Welcome () {
  ...
  return (
    <S.Box
      ...
      // <if expr="is_catsxp_origin_branded">
      className="catsxp-origin"
      // </if>
    >
      <div className="view-logo-box">
        <img src={catsxpLogoUrl} />
      </div>
      <div className={classnames({ 'view-content': true, 'initial': shouldPlayAnimations })}>
        <div className="view-header-box">
          <div className="view-details">
            <h1 className="view-title">{getLocale('catsxpWelcomeTitle')}</h1>
            <p className="view-desc">{getLocale('catsxpWelcomeDesc')}</p>
          </div>
        </div>
        <S.ActionBox>
          <Button
            ...
            size="large"
          >
            {getLocale('catsxpWelcomeSetDefaultButtonLabel')}
          </Button>
          <Button
            kind="plain-faint"
            onClick={handleSkip}
