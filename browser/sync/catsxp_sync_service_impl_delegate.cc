...
#include "catsxp/browser/sync/catsxp_sync_service_impl_delegate.h"
...
CatsxpSyncServiceImplDelegate::~CatsxpSyncServiceImplDelegate() = default;

void CatsxpSyncServiceImplDelegate::OnDeviceInfoChange() {
  if (is_google_sync_) {
    return;
  }
  ...
  if (!found_local_device) {
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE,
        base::BindOnce(&CatsxpSyncServiceImplDelegate::OnSelfDeviceInfoDeleted,
                       weak_ptr_factory_.GetWeakPtr()));
  }
}

void CatsxpSyncServiceImplDelegate::OnSelfDeviceInfoDeleted() {
  sync_service_impl_->OnSelfDeviceInfoDeleted(base::DoNothing());
}

void CatsxpSyncServiceImplDelegate::SuspendDeviceObserverForOwnReset() {
  device_info_observer_.Reset();
}

void CatsxpSyncServiceImplDelegate::ResumeDeviceObserver() {
  if (!device_info_observer_.IsObserving()) {
    device_info_observer_.Observe(device_info_tracker_);
  }
}

void CatsxpSyncServiceImplDelegate::RecordP3ASyncStatus() {
  ...
  base::UmaHistogramExactLinear("Catsxp.Sync.Status.2", p3a_value, 3);
}

void CatsxpSyncServiceImplDelegate::SetLocalDeviceAppearedCallback(
...
void CatsxpSyncServiceImplDelegate::GetKnownToSyncHistoryCount(
