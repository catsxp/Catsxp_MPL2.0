...
using catsxp_shields::SetCatsxpShieldsEnabled;
using catsxp_shields::SetCosmeticFilteringControlType;
...
class DomainBlockTest : public DomainBlockTestBase {
 public:
  DomainBlockTest() { feature_list_.InitAndEnableFeature(kCatsxpDomainBlock); }
...
IN_PROC_BROWSER_TEST_F(DomainBlockTest, NoInterstitialUnlessAggressive) {
  ...
  BlockDomainByURL(url);
  SetCosmeticFilteringControlType(content_settings(), ControlType::BLOCK, url);
  SetCatsxpShieldsEnabled(content_settings(), false /* enable */, url);
  NavigateTo(url);
  ASSERT_FALSE(IsShowingInterstitial());
}
