...
#include "catsxp/browser/ui/webui/catsxp_settings_ui.h"
#include "catsxp/components/catsxp_shields/content/browser/ad_block_service.h"
#include "catsxp/components/catsxp_shields/core/browser/ad_block_custom_resource_provider.h"
...
class AdblockCustomResourcesTest : public AdBlockServiceTest {
 public:
  AdblockCustomResourcesTest() {
    CatsxpSettingsUI::ShouldExposeElementsForTesting() = true;
  }

  ~AdblockCustomResourcesTest() override {
    CatsxpSettingsUI::ShouldExposeElementsForTesting() = false;
  }
...
IN_PROC_BROWSER_TEST_F(AdblockCustomResourcesTest, Add) {
  ...
  NavigateToURL(GURL("catsxp://settings/shields/filters"));
...
IN_PROC_BROWSER_TEST_F(AdblockCustomResourcesTest, Edit) {
  ...
  base::test::TestFuture<
      catsxp_shields::AdBlockCustomResourceProvider::ErrorCode>
      result;
  g_catsxp_browser_process->ad_block_service()
      ->custom_resource_provider()
      ->AddResource(profile()->GetPrefs(),
                    ...
                    result.GetCallback());
  EXPECT_EQ(catsxp_shields::AdBlockCustomResourceProvider::ErrorCode::kOk,
            result.Get());
...
IN_PROC_BROWSER_TEST_F(AdblockCustomResourcesTest, NameConflicts) {
  ...
  constexpr const char kCatsxpFix[] = "window.test = 'default-script'";
  constexpr const char kCatsxpFixResource[] = R"json(
    [{
      "name": "user-Fix.js",
  ...
  UpdateAdBlockResources(base::ReplaceStringPlaceholders(
      kCatsxpFixResource, {base::Base64Encode(kCatsxpFix)}, nullptr));
...
IN_PROC_BROWSER_TEST_F(AdblockCustomResourcesTest, NameCases) {
  EnableDeveloperMode(true);
  auto* provider =
      g_catsxp_browser_process->ad_block_service()->custom_resource_provider();
...
IN_PROC_BROWSER_TEST_F(AdblockCustomResourcesTest, TwoProfiles) {
  ...
  content::NavigateToURLBlockUntilNavigationsComplete(
      second_web_contents, GURL("catsxp://settings/shields/filters"), 1, true);
  ...
  // is disabled.
  EXPECT_FALSE(
      catsxp_shields::IsDeveloperModeEnabled(second_profile.GetPrefs()));
