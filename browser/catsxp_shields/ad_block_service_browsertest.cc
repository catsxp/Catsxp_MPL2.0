...
#include "catsxp/browser/catsxp_shields/ad_block_service_browsertest.h"
...
#include "catsxp/app/catsxp_command_ids.h"
#include "catsxp/browser/catsxp_browser_process.h"
#include "catsxp/browser/catsxp_shields/ad_block_browser_test_helper.h"
#include "catsxp/browser/net/catsxp_ad_block_tp_network_delegate_helper.h"
#include "catsxp/components/catsxp_shields/content/browser/ad_block_custom_filters_provider.h"
#include "catsxp/components/catsxp_shields/content/browser/ad_block_engine.h"
#include "catsxp/components/catsxp_shields/content/browser/ad_block_engine_wrapper.h"
...
#include "catsxp/components/catsxp_shields/content/browser/ad_block_subscription_service_manager_observer.h"
...
#include "catsxp/components/de_amp/common/pref_names.h"
#include "catsxp/components/playlist/core/common/buildflags/buildflags.h"
#include "catsxp/components/speedreader/common/buildflags/buildflags.h"
...
#include "catsxp/browser/speedreader/speedreader_service_factory.h"
#include "catsxp/components/speedreader/speedreader_service.h"  // nogncheck
#include "third_party/blink/public/common/web_preferences/web_preferences.h"
#endif

#if BUILDFLAG(ENABLE_PLAYLIST)
#include "catsxp/browser/playlist/playlist_service_factory.h"
#include "catsxp/components/playlist/content/browser/playlist_background_web_contentses.h"
#include "catsxp/components/playlist/content/browser/playlist_service.h"
...
using catsxp_shields::features::kCatsxpAdblockCnameUncloaking;
using catsxp_shields::features::kCatsxpAdblockCollapseBlockedElements;
using catsxp_shields::features::kCatsxpAdblockCosmeticFiltering;
using catsxp_shields::features::kCatsxpAdblockDefault1pBlocking;
using catsxp_shields::features::kCatsxpAdblockProceduralFiltering;
using catsxp_shields::features::kCatsxpAdblockScriptletDebugLogs;
using catsxp_shields::features::kCosmeticFilteringJsPerformance;
...
void AdBlockServiceTest::SetUpInProcessBrowserTestFixture() {
  ...
  ad_block_test_helper_ =
      std::make_unique<catsxp_shields::AdBlockBrowserTestHelper>();
}
...
void AdBlockServiceTest::SetUpOnMainThread() {
  ...
  catsxp_shields::SetCosmeticFilteringControlType(
      content_settings(), catsxp_shields::ControlType::BLOCK, GURL());
}

void AdBlockServiceTest::PreRunTestOnMainThread() {
  PlatformBrowserTest::PreRunTestOnMainThread();
  ASSERT_TRUE(catsxp_shields::WaitForAdBlockServiceThreads());
  ...
  ASSERT_TRUE(base::test::RunUntil([&]() {
    return g_catsxp_browser_process->ad_block_service()
        ->IsFilterListLoadedForTesting(true);
  })) << "Timeout waiting for initial engine creation";

  histogram_tester_.ExpectTotalCount(
      "Catsxp.Adblock.MakeEngineWithRules.Default", 1);
  InstallDefaultAdBlockComponent();
  histogram_tester_.ExpectTotalCount(
      "Catsxp.Adblock.MakeEngineWithRules.Default", 2);
...
void AdBlockServiceTest::TearDownOnMainThread() {
  ...
  catsxp::SetAdblockCnameHostResolverForTesting(nullptr);
...
catsxp_shields::AdBlockComponentServiceManager*
AdBlockServiceTest::component_service_manager() {
  return g_catsxp_browser_process->ad_block_service()
...
void AdBlockServiceTest::AddNewRules(const std::string& rules,
  ...
  ASSERT_TRUE(catsxp_shields::WaitForAdBlockServiceThreads());
  auto source_provider = std::make_unique<catsxp_shields::TestFiltersProvider>(
  ...
  catsxp_shields::AdBlockServiceTestObserver observer(ad_block_service);
...
void AdBlockServiceTest::UpdateAdBlockResources(const std::string& resources) {
  ...
  catsxp_shields::AdBlockService* service =
      g_catsxp_browser_process->ad_block_service();
...
void AdBlockServiceTest::UpdateAdBlockInstanceWithRules(
  ...
  auto& provider =
      component_providers.at(catsxp_shields::kDefaultAdblockFiltersListUuid);
  EXPECT_TRUE(provider);
  provider->OnComponentReady(component_path);

  catsxp_shields::AdBlockServiceTestObserver observer(service);
  observer.WaitForDefault();
}

void AdBlockServiceTest::EnableDeveloperMode(bool enabled) {
  profile()->GetPrefs()->SetBoolean(catsxp_shields::prefs::kAdBlockDeveloperMode,
                                    enabled);
}
  ...
  g_catsxp_browser_process->ad_block_service()
      ->AsyncCallAndReplyWithResult<bool>(
          base::BindLambdaForTesting(
              [tag](catsxp_shields::AdBlockEngineWrapper* wrapper) {
...
void AdBlockServiceTest::InstallComponent(
  ...
  auto* service = g_catsxp_browser_process->ad_block_service();
  std::vector<catsxp_shields::FilterListCatalogEntry> filter_list_catalog(
...
void AdBlockServiceTest::InstallDefaultAdBlockComponent() {
  auto catalog_entry = catsxp_shields::FilterListCatalogEntry(
      "default", "", "Catsxp Ad Block Updater", {}, "",
      "Default lists for Catsxp Browser", true, true, true, 0, {},
...
void AdBlockServiceTest::SetSubscriptionIntervals() {
  ...
  auto* ad_block_service = g_catsxp_browser_process->ad_block_service();
...
void AdBlockServiceTest::ShieldsDown(const GURL& url) {
  catsxp_shields::SetCatsxpShieldsEnabled(content_settings(), false, url);
}

void AdBlockServiceTest::DisableAggressiveMode() {
  catsxp_shields::SetCosmeticFilteringControlType(
      content_settings(), catsxp_shields::ControlType::BLOCK_THIRD_PARTY,
      GURL());
}
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, DefaultStartupWithCookieList) {
  ...
  histogram_tester_.ExpectTotalCount(
      "Catsxp.Adblock.MakeEngineWithRules.Default", 2);
  histogram_tester_.ExpectTotalCount(
      "Catsxp.Adblock.MakeEngineWithRules.Additional", 1);

  // Loads the default list first, then the additional cookie list.
  InstallRegionalAdBlockComponent(catsxp_shields::kCookieListUuid, true);
  ...
  histogram_tester_.ExpectTotalCount(
      "Catsxp.Adblock.MakeEngineWithRules.Default", 2);
  histogram_tester_.ExpectTotalCount(
      "Catsxp.Adblock.MakeEngineWithRules.Additional", 2);
}
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, SubFrameShieldsOff) {
  ...
  content::RunAllTasksUntilIdle();
  EXPECT_EQ(profile()->GetPrefs()->GetUint64(kAdsBlocked), 0ULL);
  catsxp_shields::SetCatsxpShieldsEnabled(content_settings(), true, url);
}
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, ServiceWorkerRequest) {
  ...
  // https://github.com/catsxp/catsxp-browser/issues/14087
  // EXPECT_EQ(profile()->GetPrefs()->GetUint64(kAdsBlocked), 1ULL);
}
...
class TestAdBlockSubscriptionServiceManagerObserver
    : public catsxp_shields::AdBlockSubscriptionServiceManagerObserver {
 public:
  ...
  explicit TestAdBlockSubscriptionServiceManagerObserver(
      catsxp_shields::AdBlockSubscriptionServiceManager* sub_service_manager)
 ...
 private:
  ...
  raw_ptr<catsxp_shields::AdBlockSubscriptionServiceManager>
...
// TODO(https://github.com/catsxp/catsxp-browser/issues/33506)
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest,
  ...
  auto* sub_service_manager = g_catsxp_browser_process->ad_block_service()
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, CnameCloakedRequestsGetBlocked) {
  ...
  catsxp::SetAdblockCnameHostResolverForTesting(&resolver);
...
class CnameUncloakingFlagDisabledTest : public AdBlockServiceTest {
 public:
  CnameUncloakingFlagDisabledTest() {
    feature_list_.InitAndDisableFeature(kCatsxpAdblockCnameUncloaking);
  }
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, SocialButttonAdBlockTagTest) {
  UpdateAdBlockInstanceWithRules(
      base::StrCat({"||example.com^$tag=", catsxp_shields::kFacebookEmbeds}));
  GURL tab_url = embedded_test_server()->GetURL("b.com", kAdBlockTestPage);
  g_catsxp_browser_process->ad_block_service()->EnableTag(
      catsxp_shields::kFacebookEmbeds, true);
  ASSERT_TRUE(catsxp_shields::WaitForAdBlockServiceThreads());
  GURL resource_url =
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, ResetPreservesTags) {
  ...
  ASSERT_TRUE(catsxp_shields::WaitForAdBlockServiceThreads());
  UpdateAdBlockInstanceWithRules("");
  AssertTagExists(catsxp_shields::kFacebookEmbeds, true);
}
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, TagPrefsControlTags) {
  ...
  AssertTagExists(catsxp_shields::kFacebookEmbeds, true);
  AssertTagExists(catsxp_shields::kTwitterEmbeds, true);
  AssertTagExists(catsxp_shields::kLinkedInEmbeds, false);

  // Toggling prefs once is reflected in the adblock client.
  local_state()->SetBoolean(catsxp_shields::prefs::kLinkedInEmbedControlType,
                            true);
  ASSERT_TRUE(catsxp_shields::WaitForAdBlockServiceThreads());
  AssertTagExists(catsxp_shields::kFacebookEmbeds, true);
  AssertTagExists(catsxp_shields::kTwitterEmbeds, true);
  AssertTagExists(catsxp_shields::kLinkedInEmbeds, true);

  local_state()->SetBoolean(catsxp_shields::prefs::kFBEmbedControlType, false);
  ASSERT_TRUE(catsxp_shields::WaitForAdBlockServiceThreads());
  AssertTagExists(catsxp_shields::kFacebookEmbeds, false);
  AssertTagExists(catsxp_shields::kTwitterEmbeds, true);
  AssertTagExists(catsxp_shields::kLinkedInEmbeds, true);

  local_state()->SetBoolean(catsxp_shields::prefs::kTwitterEmbedControlType,
                            false);
  ASSERT_TRUE(catsxp_shields::WaitForAdBlockServiceThreads());
  AssertTagExists(catsxp_shields::kFacebookEmbeds, false);
  AssertTagExists(catsxp_shields::kTwitterEmbeds, false);
  AssertTagExists(catsxp_shields::kLinkedInEmbeds, true);
  ...
  local_state()->SetBoolean(catsxp_shields::prefs::kFBEmbedControlType, true);
  ASSERT_TRUE(catsxp_shields::WaitForAdBlockServiceThreads());
  AssertTagExists(catsxp_shields::kFacebookEmbeds, true);
  AssertTagExists(catsxp_shields::kTwitterEmbeds, false);
  AssertTagExists(catsxp_shields::kLinkedInEmbeds, false);
...
class CollapseBlockedElementsFlagDisabledTest : public AdBlockServiceTest {
 public:
  CollapseBlockedElementsFlagDisabledTest() {
    feature_list_.InitAndDisableFeature(kCatsxpAdblockCollapseBlockedElements);
  }
...
class Default1pBlockingFlagDisabledTest : public AdBlockServiceTest {
 public:
  Default1pBlockingFlagDisabledTest() {
    feature_list_.InitAndDisableFeature(kCatsxpAdblockDefault1pBlocking);
  }
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, HiddenListsNotPresented) {
  std::vector<catsxp_shields::FilterListCatalogEntry> filter_list_catalog;
  filter_list_catalog.push_back(catsxp_shields::FilterListCatalogEntry(
...
class CosmeticFilteringFlagDisabledTest : public AdBlockServiceTest {
 public:
  CosmeticFilteringFlagDisabledTest() {
    feature_list_.InitAndDisableFeature(kCatsxpAdblockCosmeticFiltering);
  }
...
class ProceduralFilteringFlagDisabledTest : public AdBlockServiceTest {
 public:
  ProceduralFilteringFlagDisabledTest() {
    feature_list_.InitAndDisableFeature(kCatsxpAdblockProceduralFiltering);
  }
...
IN_PROC_BROWSER_TEST_F(CosmeticFilteringPlaylistFlagEnabledTest,
  ...
  catsxp_shields::SetCosmeticFilteringControlType(
      content_settings(), catsxp_shields::ControlType::ALLOW, url);
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, CosmeticFilteringDisabled) {
  catsxp_shields::SetCosmeticFilteringControlType(
      content_settings(), catsxp_shields::ControlType::ALLOW, GURL());
  UpdateAdBlockInstanceWithRules(
      "b.com###ad-banner\n"
      "##.ad");
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, CosmeticFilteringSimple) {
  ...
  ASSERT_TRUE(result_first.is_ok());
  ...
  ASSERT_TRUE(result_third.is_ok());
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, CosmeticFilteringDynamicCustom) {
  ASSERT_TRUE(g_catsxp_browser_process->ad_block_service()
...
class ScriptletDebugLogsFlagEnabledTest : public AdBlockServiceTest {
 public:
  ScriptletDebugLogsFlagEnabledTest() {
    feature_list_.InitAndEnableFeature(kCatsxpAdblockScriptletDebugLogs);
  }
 ...
 public:
  explicit CookieListPrefObserver(PrefService* local_state) {
    ...
    pref_change_registrar_.Add(
        catsxp_shields::prefs::kAdBlockRegionalFilters,
...
// Test that the `catsxp-adblock-default-1p-blocking` flag forces the Cookie
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, ListEnabled) {
  InstallRegionalAdBlockComponent(catsxp_shields::kCookieListUuid, false);
  ...
  // Disable the filter list.
  {
    ...
    component_service_manager()->EnableFilterList(
        catsxp_shields::kCookieListUuid, false);
    pref_observer.Wait();
  }
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, ContentPicker) {
  ...
  static constexpr char kPickerIsInjected[] =
      "document.getElementById('catsxp-element-picker') != null";
  ASSERT_FALSE(
  ...
  ASSERT_TRUE(content::ExecJs(web_contents(),
                              ...
                              ISOLATED_WORLD_ID_CATSXP_INTERNAL));
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest,
  ...
  catsxp_shields::SetAllowElementBlockerInPrivateModeEnabled(local_state(),
                                                            true);
  const GURL tab_url =
      embedded_test_server()->GetURL("a.com", "/cosmetic_filtering.html");
  Browser* private_browser = CreateIncognitoBrowser();
  ...
  AdblockOnlyModeFilterListsTest() {
    feature_list_.InitWithFeatures(
        {catsxp_shields::features::kAdblockOnlyMode,
         catsxp_shields::features::kCatsxpAdblockShowHiddenComponents},
        {});
  }
...
IN_PROC_BROWSER_TEST_F(AdblockOnlyModeFilterListsTest,
  ...
  // Adblock-Only mode is disabled by default, so all filter lists are enabled.
  {
    EXPECT_TRUE(component_service_manager()->IsFilterListEnabled(
        catsxp_shields::kDefaultAdblockFiltersListUuid));
    EXPECT_TRUE(component_service_manager()->IsFilterListEnabled(
        catsxp_shields::kCookieListUuid));
...
IN_PROC_BROWSER_TEST_F(AdblockOnlyModeFilterListsTest,
  ...
  component_service_manager()->EnableFilterList(
      catsxp_shields::kDefaultAdblockFiltersListUuid, false);
