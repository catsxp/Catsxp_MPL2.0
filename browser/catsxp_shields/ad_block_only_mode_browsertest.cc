...
#include "catsxp/components/debounce/core/common/pref_names.h"
#include "catsxp/components/global_privacy_control/pref_names.h"
#include "catsxp/components/query_filter/common/pref_names.h"
...
class AdblockOnlyModeBrowserTestBase : public PlatformBrowserTest {
 public:
  ...
  catsxp_shields::CatsxpShieldsSettingsService* catsxp_shields_settings() {
    return CatsxpShieldsSettingsServiceFactory::GetForProfile(GetProfile());
  }
  ...
  void SetupSettingsCausingWebCompatIssues() {
    ...
    // Enable Catsxp Fingerprinting protection.
    ...
    catsxp_shields_settings()->SetForgetFirstPartyStorageEnabled(true, GURL());
  ...
  void VerifySettingsCausingWebCompatIssues() {
    ...
    // Verify that Catsxp Fingerprinting protection is enabled.
    ...
    EXPECT_FALSE(catsxp_shields::AreReferrersAllowed(host_content_settings_map(),
                                                    GURL()));

    // Verify that forget first-party storage is enabled.
    EXPECT_TRUE(
        catsxp_shields_settings()->GetForgetFirstPartyStorageEnabled(GURL()));
  ...
  void VerifyAdblockOnlyModeSettings() {
    ...
    // Verify that Catsxp Fingerprinting protection is disabled.
    EXPECT_EQ(GetFingerprintingControlType(host_content_settings_map(), GURL()),
              ControlType::ALLOW);
    ...
    EXPECT_TRUE(catsxp_shields::AreReferrersAllowed(host_content_settings_map(),
                                                   GURL()));
