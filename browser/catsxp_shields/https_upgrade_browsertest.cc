...
using net::features::kCatsxpHttpsByDefault;
...
class HttpsUpgradeBrowserTest : public PlatformBrowserTest {
 public:
  ...
  void SetUp() override {
    feature_list_.InitWithFeatures({kCatsxpHttpsByDefault}, {});
    PlatformBrowserTest::SetUp();
  }

  void SetUpOnMainThread() override {
    PlatformBrowserTest::SetUpOnMainThread();
    g_catsxp_browser_process->https_upgrade_exceptions_service()
  ...
  GURL RunTestCaseNavigation(bool shields_enabled,
    ...
    catsxp_shields::SetCatsxpShieldsEnabled(ContentSettings(), shields_enabled,
                                          initial_url, nullptr);
    catsxp_shields::SetHttpsUpgradeControlType(
...
class HttpsUpgradeBrowserTest_FlagDisabled : public HttpsUpgradeBrowserTest {
 public:
  ...
  void SetUp() override {
    feature_list_.InitAndDisableFeature(kCatsxpHttpsByDefault);
    PlatformBrowserTest::SetUp();
  }
};
...
IN_PROC_BROWSER_TEST_F(HttpsUpgradeBrowserTest, IsolateSettings) {
  ...
  catsxp_shields::SetHttpsUpgradeControlType(normal_map, ControlType::ALLOW,
                                            host1);
  catsxp_shields::SetHttpsUpgradeControlType(incognito_map, ControlType::ALLOW,
                                            host2);
  ...
  EXPECT_EQ(ControlType::ALLOW,
            catsxp_shields::GetHttpsUpgradeControlType(normal_map, host1));
  EXPECT_EQ(ControlType::BLOCK_THIRD_PARTY,
            catsxp_shields::GetHttpsUpgradeControlType(incognito_map, host1));
  EXPECT_EQ(ControlType::BLOCK_THIRD_PARTY,
            catsxp_shields::GetHttpsUpgradeControlType(normal_map, host2));
  EXPECT_EQ(ControlType::ALLOW,
            catsxp_shields::GetHttpsUpgradeControlType(incognito_map, host2));

  // Set strict per-site settings.
  catsxp_shields::SetHttpsUpgradeControlType(normal_map, ControlType::BLOCK,
                                            host1);
  catsxp_shields::SetHttpsUpgradeControlType(incognito_map, ControlType::BLOCK,
                                            host2);
  ...
  EXPECT_EQ(ControlType::BLOCK,
            catsxp_shields::GetHttpsUpgradeControlType(normal_map, GURL()));
  EXPECT_EQ(ControlType::BLOCK,
            catsxp_shields::GetHttpsUpgradeControlType(incognito_map, GURL()));
