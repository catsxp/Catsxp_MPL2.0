...
const char* TypeToString(NTPBackgroundPrefs::Type type) {
  ...
  switch (type) {
    case NTPBackgroundPrefs::Type::kCatsxp:
      return "catsxp";
...
NTPBackgroundPrefs::Type StringToType(const std::string& type_string) {
  // See class description for details.
  if (type_string == "catsxp") {
    return NTPBackgroundPrefs::Type::kCatsxp;
  }
...
void NTPBackgroundPrefs::RegisterPref(
  ...
  base::DictValue dict;
  dict.Set(kTypeKey, TypeToString(Type::kCatsxp));
...
bool NTPBackgroundPrefs::IsCatsxpType() const {
  return GetType() == Type::kCatsxp;
}
...
bool NTPBackgroundPrefs::ShouldUseRandomValue() const {
  ...
  return optional_bool.value_or(IsCatsxpType());
}
