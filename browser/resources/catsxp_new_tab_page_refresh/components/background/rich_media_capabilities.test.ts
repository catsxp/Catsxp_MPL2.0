...
describe('readIncomingMessage', () => {
  ...
  describe('valid richMediaMakeCatsxpSearchDefault messages', () => {
    test('returns correct message for richMediaMakeCatsxpSearchDefault', () => {
      const result = readIncomingMessage({
        type: 'richMediaMakeCatsxpSearchDefault',
      })
      expect(result).toEqual({ type: 'richMediaMakeCatsxpSearchDefault' })
    })
  })
...
describe('dispatchIncomingMessage', () => {
  ...
  beforeEach(() => {
    mockCapabilities = {
      ...
      openCatsxpSearch: jest.fn(),
      queryCatsxpSearchAutocomplete: jest.fn(),
      hideCatsxpSearchBox: jest.fn(),
      makeCatsxpSearchDefault: jest.fn(),
    }
  })
  ...
  describe('richMediaQueryCatsxpSearchAutocomplete message', () => {
    it('should call queryCatsxpSearchAutocomplete with the query value', () => {
      dispatch({
      ...
      expect(
        mockCapabilities.queryCatsxpSearchAutocomplete,
      ).toHaveBeenCalledWith('test query')

      expect(mockCapabilities.notifyAdEvent).not.toHaveBeenCalled()
      expect(mockCapabilities.openDestinationUrl).not.toHaveBeenCalled()
      expect(mockCapabilities.openCatsxpSearch).not.toHaveBeenCalled()
      expect(mockCapabilities.hideCatsxpSearchBox).not.toHaveBeenCalled()
    })
  })

  describe('richMediaOpenCatsxpSearchWithQuery message', () => {
    it('should call notifyAdEvent with kClicked and openCatsxpSearch', () => {
      dispatch({
      ...
      expect(mockCapabilities.openCatsxpSearch).toHaveBeenCalledWith(
        'search query',
      )
    })
  })

  describe('richMediaHideCatsxpSearchBox message', () => {
    it('should call hideCatsxpSearchBox', () => {
      dispatch({ type: 'richMediaHideCatsxpSearchBox' })

      expect(mockCapabilities.hideCatsxpSearchBox).toHaveBeenCalled()
    })
  })

  describe('richMediaMakeCatsxpSearchDefault message', () => {
    it('should call makeCatsxpSearchDefault', () => {
      dispatch({ type: 'richMediaMakeCatsxpSearchDefault' })

      expect(mockCapabilities.makeCatsxpSearchDefault).toHaveBeenCalled()
    })
  })
})
