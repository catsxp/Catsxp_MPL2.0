...
import { SponsoredRichMediaAdEventHandler } from 'gen/catsxp/components/ntp_background_images/browser/mojom/ntp_background_images.mojom.m.js'
...
export function createBackgroundStore() {
  ...
  store.update({
    catsxpBackgrounds: preloadedBackgrounds,
  ...
  async function updateCatsxpBackgrounds() {
    const { backgrounds } = await handler.getCatsxpBackgrounds()
    store.update({
      catsxpBackgrounds: backgrounds.length ? backgrounds : preloadedBackgrounds,
    })
  }
