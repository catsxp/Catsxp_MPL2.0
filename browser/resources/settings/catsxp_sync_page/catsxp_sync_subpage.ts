...
import './catsxp_sync_configure.js'
import './catsxp_sync_setup.js'
...
import {
  CatsxpSyncBrowserProxy,
  CatsxpSyncStatus
} from './catsxp_sync_browser_proxy.js'
import {SyncPrefs} from '/shared/settings/people_page/sync_browser_proxy.js';

import {getTemplate} from './catsxp_sync_subpage.html.js'

type PageStatus = 'configure' | 'setup' | 'spinner'

const SettingCatsxpSyncSubpageBase =
  I18nMixin(RouteObserverMixin(PolymerElement))

class SettingCatsxpSyncSubpage extends SettingCatsxpSyncSubpageBase {
  static get is() {
    return 'settings-catsxp-sync-subpage'
  }
  ...
  private declare syncStatus: CatsxpSyncStatus
  ...
  private catsxpSyncBrowserProxy_: CatsxpSyncBrowserProxy =
  ...
  override currentRouteChanged() {
    ...
    if (router.getCurrentRoute() === router.getRoutes().CATSXP_SYNC_SETUP) {
      this.onNavigateToPage_()
      return
    }

    if (router.getRoutes().CATSXP_SYNC_SETUP.
  ...
  private onNavigateToPage_() {
    const router = Router.getInstance()
    assert(router.getCurrentRoute() === router.getRoutes().CATSXP_SYNC_SETUP)
    if (this.beforeunloadCallback_) {
      return
    }
  ...
  private onSetupSuccess_() {
    ...
    const router = Router.getInstance()
    if (router.getCurrentRoute() === router.getRoutes().CATSXP_SYNC_SETUP) {
      router.navigateTo(router.getRoutes().CATSXP_SYNC)
    }
  }
  ...
  async onRejoin_(_e: Event) {
    ...
    await this.catsxpSyncBrowserProxy_.resetSyncChain()
...
customElements.define(
  SettingCatsxpSyncSubpage.is, SettingCatsxpSyncSubpage)
